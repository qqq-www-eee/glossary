...
3. В правой части экрана пользователь может:
    1. создавать новые наименования работ и новые Виды работ (пустые папки). Если название создаваемой работы уже есть в дефолтном шаблоне, система не ругается и наименование работы создается в правой части; при этом соответствующее наименование работы все равно перестает отображаться в левой части ui;
    2. перемещать одну/несколько выбранных наименований работ из различных локаций в некоторую существующую папку (корень отображается в списке папок как пункт "Поместить вне видов работ");
    3. удалять выбранные наименования работ и выбранные виды работ (папки). Наименования работ перестанут отображаться в правой части ui и начнут отображаться в левой части ui, если они представлены в дефолтном системном шаблоне (если удаляемая работа отсутствует в шаблоне, т. е. была создана пользователем, она просто удаляется).
    Если папка "Вид работ" в списке работ шахматки:
        1. пустая, по нажатии на кнопку "Удалить" она удаляется;
        2. имеет внутри себя хотя бы одно наименование работ, при нажатии на кнопку "Удалить" удаляется только содержимое папки.
    5. работать со списком работ:
        1. Клонировать список работ из другой шахматки;
        2. Очистить список.
        3. ~~Экспортировать список работ;~~
        4. ~~Импортировать список работ - чаще всего сейчас он будет плоский. В любом случае, нужен контракт;~~
...
6. Пользователь с правами "Чтение" может просматривать список работ, но не может ничего в нем редактировать. Также он может клонировать список работ к себе в шахматки. Пользователь с правами "Редактирование" может все то же, что владелец, в т. ч. числе редактировать список работ и клонировать его к себе в шахматки.
