...
3. При нахождении пользователя в разделе "Список работ":
    1. если в списке работ шахматки нет ни одной работы, пользователь видит в области дефолтных работ перечень наименований всех работ базового шаблона (work_list_templates, [данные тут](https://docs.google.com/spreadsheets/d/1KBcwx9LyvH2fK0nzmJoen7atJCyp81WO-frnL3uYpXg/edit#gid=732289179)), а в области работ шахматки - заглушку.
...
6. Пользователь-"Владелец" и пользователь-"Редактор" могут:
    ...
    3. выбирать одну или несколько наименований работ в дефолтном списке работ для (массового) добавления в список шахматки; после успешного добавления:
        ...
        2. при переносе работы она перестает отображаться в старой области и начинает отображаться в новой (хотя из таблицы work_list_templates запись не удаляется);
    ...
    4. выбирать одну или несколько наименований работ в списке работ шахматки (для удаления, для перемещения):
        1. Удаление:
            ...
            2. если к наименованию работ не привязаны работы (таски, задачи) ИЛИ при прямом согласии пользователя в модальном окне:
                1. Из дефолтного списка работ (таблицы work_list_templates) наименование работы никогда не удаляется.
                2. Для всех наименований работ из списка дефолтных:
                    1. Из таблицы work соответствующие наименования работ удаляются.
                    2. Из таблицы task соответствующие работы каскадом также удаляются.
                    2. На fron-end перестает выполняться условие на присутствие этих дефолтных работ в области работ шахматки (справа), поэтому в области дефолтных работ (слева) они начинают вновь отображаться.
                3. Для всех наименований работ, введенных пользователем:
                    1. Из таблицы work соответствующие наименования работ удаляются.
                    2. Из таблицы task соответствующие работы каскадом также удаляются.
            ...
            4. после успешного переноса наименования работы из шахматки в системный список работ (aka "Удаление"):
        ...
        2. Перемещение: переносить наименования работ в списке шахматки между разными уровнями вложенности:
    ...
    5. если в списке работ шахматки выбрано несоколько чекбоксов с работами или видами работ, это никак не влияет на процессы работы:
        1. кнопки "Создать" ("Работу", "Вид работ");
        2. кнопки "Список" ("Клонирование списка работ из другой шахматки", "Экспортировать в файл (xlsx)", "Импортировать из файла", "Очистить список")
    6. система никак не препятствует сводобному выбору работ одновременно в двух частях экрана - это разные списки;
    7. создавать новую работу непосредственно в списке шахматки (тут важно, что разный флоу у тех, что из шаблона и тех, что добавил пользователь):
