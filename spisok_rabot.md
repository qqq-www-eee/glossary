...
2. **Вид работ** (группа работ) - несколько наименований работ, объединенных пользователем по какому-то принципу в группу. Отображается в UI как папка. Сейчас система позволяет создавать только один уровень вложенности, по бизнесу соответствующий "Виду работ".
3. **Работа** - проведение конкретного наименования работ в определенном помещении.  
...
2. Новую таблицу (напр. sys_work_list) создать под **системный список работ**.

### Ремарка

> Если привязывать работу к системному списку работ (sys_work_list) вместо списка работ шахматки (work), то при удалении строки из work работа (task) все еще будет ссылаться на корректное наименование в sys_work_list - данные не потеряются, получится soft-delete для работ, при добавлении работы назад все работы в шахматке заресторятся. Но:  
> 1. Не хотим перегружать логику для пользователя  
> 2. У пользователя будет версионирование и откат.  
> 3. Если связанные с наименованием работы таски не перестанут существовать, они могут появляться в нарядах. Тогда у работы напрашивается дополнительный статус "Удалена" (но статус работы - это немного про другое), это переусложнение. Если пользователю очень надо, пусть не удаляет наименование работ.
> 
> Так что предлагается работы (таски) привязывать к work, и удаление строки оттуда приведет к жесткому удалению соответствующих работ (либо в работах в work_id проставляется null - в любом случае, они нам не нужны).

### Функциональные требования

1. При переходе в шахматку (объект):
    1. пользователь может перейти в раздел "**Список работ**" через контекстное меню кнопки "Настройка";
    2. если в списке работ шахматки нет ни одной работы (на старте работы либо в случае полного удаления), система дублирет в рабочей области шахматки предложение настроить/создать список;
    3. если в списке работ шахматки нет ни одной работы, пользователь в разделе "**Список работ**" видит в области системных работ перечень наименований всех работ (sys_work_list, [сам список тут](/Дефолтные-работы.pdf)), а в области работ шахматки - заглушку.
...
3. Пользователи могут добавлять новые строки в список работ шахматки (work):
    1. из контекстного меню в левой панели интерфейса при работе с шахматкой;
