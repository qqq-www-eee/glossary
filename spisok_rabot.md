...
1. Настройка списка работ ведется в двух областях интерфейса:
    1. Левая отображает готовый системный шаблон на основе [этих данных](https://docs.google.com/spreadsheets/d/1KBcwx9LyvH2fK0nzmJoen7atJCyp81WO-frnL3uYpXg/edit#gid=732289179). Пользователь не может изменять шаблон. Шаблон не имеет иерархической структуры (плоский);
    2. Правая область отвечает за настройку текущей пользовательской шахматки. Пользователь может изменять шаблон (создавать, удалаять, перемещать). Пользователь не может редактировать конкретные наименования работ. Список работ шахматки поддерживает иерархическую структуру (вложенность папок не более 1 уровня).
2. В левой части экрана пользователь может:
    1. добавить одну/несколько выбранных работ из шаблона в список шахматки. Наименования работ перестанут отображаться в левой части ui и начнут отображаться в корне в правой части ui.
3. В правой части экрана пользователь может:
    1. **создать** новые наименования работ, новые Виды работ (пустые папки). Если название создаваемой работы уже есть в дефолтном шаблоне, то система никак не ругается и наименование работы создается в правой части; при этом соответствующее наименование работы перестает отображаться в левой части ui;
    2. **перемещать** выбранные наименования работ между папками (корень отображается в списке папок как пункт "Поместить вне видов работ");
    3. **удалять** выбранные наименования работ, выбранные виды работ (папки). При этом наименования работ перестанут отображаться в правой части ui и начнут отображаться в левой части ui, если они представлены в дефолтном системном шаблоне. Если же удаляемая работа была создана пользователем, она просто удаляется.
    Если папка "Вид работ" в списке работ шахматки:
    ...
    5. **работать со списком** работ:
        ...
        2. ~~Экспортировать список работ;~~
        3. ~~Импортировать список работ - чаще всего сейчас он будет плоский. Нужен контракт;~~
        4. Очистить список.
4. Наименования работ в шаблоне уникальны. Наименования работ в списке шахматки уникальны сквозным образом. Виды работ в списке шахматки уникальны. Работа и вид могут иметь одинаковое название. 
5. Нотификации показываются, когда
    1. невозможно переместить или удалить работы, так как ни одна работа не выбрана;
    2. работы успешно удалены.
6. Пользователь с правами "Чтение" может просматривать все, что касается списка работ, но не может ничего редактировать. Может клонировать к себе список работ из шахматки, к которой у него есть доступ. Пользователь с правами "Редактирование" может все то же, что владелец, в том числе клонировать к себе список работ из шахматки, к которой у него есть доступ.
