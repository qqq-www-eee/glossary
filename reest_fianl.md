...
1. Реестры могут находиться в статусах "Сформирован", "Удален", "Оплачен":
    1. "**Сформирован**" - дефолтный статус ('черновик').
    2. "**Оплачен**" - будет автоматически проставляться после интеграции с 1С, сейчас проставляется руками через какой-то контрол (в заглушке после кнопки "Отправить в 1С"? вместо кнопкуи "Отправить в 1С"?). Данный статус означает: система учитывает этот реестр при подсчете лимитов.
...
2. У реестров есть два важных кейса применения:
    1. Всевозможные завершенные работы за последнюю неделю. Составляет ТН (теперь уже прораб) для оплаты в бухгалтерию. Содержит всевозможные фамилии мастеров. Таким реестрам, скорее всего, будут проставлять статус "Оплачено". 
    2. Выписку по работам одного мастера: прорабам и технадзорам теперь запрещено в общие чаты общие реестры, так как в последних содержится чувствительная информация. Теперь крайне желательно иметь возможность скидывать работнику сводку в результатами только его труда. Для одного отчетного периода, этого возможно добиться с помощью фильтров.
3. Если два человека могут иметь доступ к созданию реестров и преследовать при этом разные цели (Технадзор формирует реестр на выплату, а помощник прораба хочет распечатать те работы, которые сделал Нуралиев), то два и более реестра с одной и той же работой должны существовать отдельно. НО данная работа должна быть посчитана в лимитах единожды. Поэтому основному реестру прораб проставляет статус "Оплачено", остальные остаются в статусе "Сформирован".
