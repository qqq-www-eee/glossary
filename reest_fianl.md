...
1. Система дает возможность работать как с почасовыми, так и со сдельными работами. Техническая реализация одинакова: почасовые работы вносятся через единицу измерения работы "час" в таблице unit. В work_template будут добавлены нескольких новых работы почасовых работ.  
2. При копировании работы в списке работ из шаблона в шахматку, помимо названия копируется также дефолтная расценка данной работы (из work_template.price в work.price). При создании реестра пользователь все так же может скорректировать расценку нарпямую для текущего объекта (обновится work.price).
3. Наряды могут находиться в статусах "Сформирован", "Удален", "Оплачен".
    1. "Сформирован" - дефолтный статус (по сути значит 'черновик');
    2. "Оплачен" - будет автоматически проставляться после интеграции с 1С (сейчас проставляется руками через дропдаун), означает, что система учитывает данные наряды в лимитах;
    3. "Удален" - проставляемый руками как "не предполагающий дальнейшей работы". Удаленный наряд можно восстановить (перевести сейчас в статус формирован).
4. Все реестры в статусе "Оплачено", куда вошла данная работа, отображаются
    1. в сайдбаре шахматки;
    2. в карточке работы.
5. При массовом выделении ячеек - в сайдбаре НЕ выводятся данные о реестрах. Незачем выводить кучу реестров шахматки всех мастей.
6. Добавить новый тип логов для истории изменений работы, "Задача внесена в реестр №ХХ".
7. В карточку работы должно быть можно провалиться из работы! А не только из профиля "Вид работ", очень не логично. Более того, из "Вида работ" очень хочется провалиться в обычную работу, а не в карточку. А там уже есть переход в карточку и в логи.
