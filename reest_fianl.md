...
1. Реестры на выплату могут находиться в статусах "Сформирован", "Удален", "Оплачен":
    1. "**Сформирован**" - дефолтный статус (по сути 'черновик').
    2. "**Оплачен**" - будет автоматически проставляться после интеграции с 1С, сейчас проставляется руками через некий контрол (в заглушке после кнопки "Отправить в 1С"? вместо кнопки "Отправить в 1С"? на странице реестра?). Данный статус означает: система учитывает этот реестр при подсчете лимитов.
...
4. Не хотелось бы принуждать пользователя восстановливать систему каждый раз, когда ему потребуется удаленный ранее наряд, поэтому предлагается ввести также статус "Удален", чтобы иметь возможность скрывать и снова просматривать ненужные реестры.
5. При просмотре информации о работе система отобразит все реестры в статусе "Оплачено", куда вошла данная работа:
    1. в боковой панели шахматки;
        1. при массовом выделении ячеек в боковой панели не выводятся данные о реестрах (записи об оплате);
    2. в карточке работы.
...
7. Добавить новый тип логов в историю изменений работы: "Задача внесена в реестр №ХХ".
...
10. Статусы работ, статусы замечаний и статусы реестров - разные сущности, не имеюшие между собой ничего общего, кроме того, что это статусы.
...
13. Необходима возможность поставить реестру название (комментарий) (можно создать в один день множество реестров, например, для нескольких исполнителей, в которых будут отчеты конкретно по ним)
