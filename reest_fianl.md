...
- Добавить в payment_report_task поля для хранения:
    - float **price** для хранения расценки (чтобы не считать все время total_price / volume), но это опционально (то есть можно каждый раз считать);
    - float **executor_total** - просуммированный итог этого конкретного исполнителя в этом наряде, "Накопительный итог";
    - varchar **task_status** - статус работ на момент формирования реестра;
    - ~~bool 'hasRevisions' - были ли замечания на момент формирования наряда? зачем?~~
