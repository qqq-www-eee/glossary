...
6. В карточку работы можно перейти из боковой панели в профиле работы (не только из профиля "Вид работ").
    1. Из профиль "Вид работ" очень хочется переходить сначала в профиль работы, а не в карточку работы. А уже из сайдбара работы осуществлять переход в карточку работы с логами.
...
11. Однако доступ на редактирование шахматки = доступ на создание/редактирование замечаний = доступ на создание реестров.
12. В реестры необходимо добавить новый столбец "Накопительный итог":
    1. Одна возможная реализация - новый столбец. Если Иванов встретился пять раз в реестре, то после Шага 2 формирования реестра сложить на back-end все его выплаты в данном реестре и написать в последнем столбце суммарную цифру пять раз (в соответствующих строках).  
...
1. Общие требования:
    1. Переход в раздел "Реестры на оплату" осуществляется:
        ...
        2. из шахматки через **Боковую панель**, когда в **Левой панели** выбран профиль некоторой работы (но не профиль "Замечания" и не профиль "Вид работ") - в боковой панели в этом случае приведены кликабельные **записи об оплатах** для данной работы. В записях об оплате отображены:
