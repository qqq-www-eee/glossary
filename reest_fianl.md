...
1. Реестры могут находиться в статусах "Сформирован", "Удален", "Оплачен":
    ...
    2. "**Оплачен**" - будет автоматически проставляться после интеграции с 1С, сейчас проставляется руками через некий контрол (в заглушке после кнопки "Отправить в 1С"? вместо кнопки "Отправить в 1С"? прямо в наряде?). Данный статус означает: система учитывает этот реестр при подсчете лимитов.
...
2. У реестров есть два важных кейса применения:
    1. Всевозможные завершенные работы за последнюю неделю. Составляет ТН (теперь уже прораб) для оплаты в бухгалтерии; содержит всевозможные фамилии мастеров. Таким реестрам, скорее всего, будут проставлять статус "Оплачено". 
    2. Выписка по работам одного мастера. Прорабам и технадзорам теперь запрещено скидывать в общие чаты общие реестры, так как в последних содержится чувствительная информация. Поэтому теперь крайне желательно иметь возможность скидывать работнику сводку в результатами только его труда - для одного отчетного периода этого возможно добиться с помощью фильтров.
3. Если два человека имеют доступ к созданию реестров и преследуют при этом разные цели (ТН формирует реестр на выплату, а помощник прораба хочет распечатать работы какого-то мастера), то два и более реестра с одной и той же работой вынеждены сосуществовать одновременно. Однако сама работа должна быть посчитана в лимитах единожды, поэтому есть возможность основному реестру напрямую проставить статус "Оплачено", а остальные остаются в статусе "Сформирован".
4. При отображении работы система также отображается все реестры в статусе "Оплачено", куда вошла данная работа:
...
7. К сдельным добавляются **почасовые** работы. Техническая реализация: в work_template добавляются нескольких новых почасовых работ с единицей измерения "час" из таблицы unit.  
8. Настройка спарвочника работ: при копировании работы из шаблона в шахматку помимо названия копируется также дефолтная расценка (из work_template.price в work.price).
...
12. В реестры попросили добавить новый столбец - накопительный итог...
...
13. Еще нам нужна возможность поставить все-таки название реестру! Потому что можно создать много реестров для нескольких исполнителей, в которых будут отчеты конкретно по ним. У реестра должно быть корректируемое название!
