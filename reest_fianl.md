...
1. Система работает как с почасовыми, так и со сдельными работами. Техническая реализация одинакова. В work_template добавляются нескольких новых почасовых работ с единицей измерения "час" из таблицы unit.  
2. Настройка спарвочника работ: при копировании работы из шаблона в шахматку помимо названия копируется также дефолтная расценка (из work_template.price в work.price).
    1. При создании реестра пользователь все так же сможет скорректировать расценку для текущего объекта (обновится work.price).
3. Наряды могут находиться в статусах "Сформирован", "Удален", "Оплачен":
    1. "Сформирован" - дефолтный статус (по сути 'черновик');
    2. "Оплачен" - будет автоматически проставляться после интеграции с 1С, сейчас проставляется руками через контрол. Статус означает, что система должна учитывает данные наряды при подсчете лимитов;
    3. "Удален" - реестр "не предполагает дальнейшей работы"; удаленный наряд можно восстановить (перевести в статус формирован или "Оплачен", после интеграции - только в "Сформирован").
...
8. Природа статуса "Оплачено" - два человека могут иметь доступ до создания нарядов. Они могут преследовать разные цели при создании нарядов (Технадзор реально закрывает наряд, а помощник прораба хочет распечатать только те работы, которые сделал Нуралиев, потому что Нуралиев достает прораба вопросами, сколько он получит, он хочет бумажку). Два и более наряда, в которые вошла одна и та же работа, должны мирно сосуществовать, но данная работа не должна быть посчитана дважды в лимитах! Поэтому прораб основному наряду проставляет статус "Оплачено" - и работа считается в системе как оплаченный наряд. Остальные - просто есть, их можно распечатать и так далее.
...
10. Статусы работ не имеют ничего общего со статусами нарядов. Работа может попасть во множество нарядов, а в одном наряде содержится множество работ. Кажется, будто невозможно поддерживать статусы в одном месте, так как соотношение многие-ко-многим. Статусы замечаний, опять же, живут в отдельной карточке, это логично и, в целом, интуитивно понятно. 
11. В наряды попросили добавить новый столбец - накопительный итог...
    1. Одна возможная реализация - если встретился Иванов пять раз в наряде, сложить все его выплаты в этом наряде и написать в последнем столбце наряда суммарную цифру пять раз (в соответствующих строках). У каждой из работ в наряде обязательно есть исполнитель, так что, по идее, можно вычислить это значение на back-end.  
    2. Выводить эту инфу в отдельной таблицке?
12. Еще нам нужна возможность поставить все-таки название наряду! Потому что можно создать много нарядов для нескольких исполнителей, в которых будут отчеты конкретно по ним.
13. У наряда должно быть корректируемое название!
14. У реестров есть два крайне важных кейса применеия:
    1. Все работы за последнюю неделю - с кучей фамилий. Составляет ТН (теперь уже прораб) для оплаты в бухгалтерию. Таким нарядам, скорее всего, удобно будет проставить статус "Оплачено". 
    2. Сделать выписку по работам только одного-единственного мастера. Недавно прорабам/технадзорам запретили скидывать общие наряды в чаты, так как в них содержится чувствительная информация. Теперь крайне желательно иметь возможность скидывать работнику сводку по результатам только его труда. Если мы говорим только об одном отчетном периоде, этого можно добиться с помощью фильтров.
...
3. Формирование нового реестра.
    ...
    2. Шаг 2 формирования нового реестра: Расценки
        ...
        3. Если расценка на работу с данным именем на данном объекте:
            ...
            4. По-хорошему сразу при создании работ надо указывать расценку, но мы редактируем через наряды. Так можно и цену скорректировать, и название работы не трогать.
