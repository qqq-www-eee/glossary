...
1. Система дает возможность работать как с почасовыми, так и со сдельными работами. Технически они реализуются одинаково. Почасовые - через единицу измерения работы "час" в таблице unit и добавление нескольких новых работы в work_template.  
2. В справочнике работ работы из шаблона работ должны копироваться в список шахматки вместе с дефолтной ценой (из work_template.price в work.price). При создании реестра пользователь все так же может изменить цену для текущего объекта (обновится work.price).
3. Наряды могут находиться в статусах "Сформирован", "Удален", "Оплачен". "Сформирован" - дефолтный статус (по сути черновик), "Оплачен" - автоматически проставляемый после интеграции с 1С (сейчас проставляется руками), "Удален" - проставляемый руками как "не предполагающий дальнейшей работы". Пользователь сейчас может свободно менять статус наряда через дропдаун.
4. В сайдбаре шахматки отображаются все реестры в статусе "Оплачено", куда вошла данная работа. Также отображаются в карточке работы.
