...
12. Частичных оплат нет:
    1. Каждая работа попадает в реестры сколь угодно много раз; работы нельзя "закрыть" какой-либо оплатой;
    2. При формировании реестра новые записи о работах "на остаток" не создаются ни руками, ни автоматически, никак. Живет одна таска и всегда попадает в реестры (разве что, пока ее бэкэнд как-нибудь не убьет);
...
13. Критерии попадания работы в реестр упрощаются с:
    1. Статус "Завершено";
...
14. Сейчас в мобильных устройствах функционал реестров не реализуется. ~~Будет необходим в мобильном приложении как только будет реализована интеграция с 1C. Он будет работать только если устройство онлайн.~~
...
- Добавить в payment_report_task поля для хранения:
    - расценки (чтобы не считать все время total_price / volume), но это опционально (то есть можно каждый раз считать);
    - executor_total - просуммированный итог этого конкретного исполнителя в этом наряде, "Накопительный итог";
    - varchar 'task_status' - статус работ на момент формирования реестра;
    - ~~bool 'hasRevisions' - были ли замечания на момент формирования наряда??? зачем это?~~

### Action points

1. Добавить в **work_template** поле **price**.
2. Добавить в шаблон work_template почасовые работы (9 работ):  
...
3. Добавить в **work_template** остальные шаблонные работы (сдельные) с расценками согласно [таблице]().

### LATER 

Подумать еще, будет ли у нас что-то вроде поиска/фильтрации в готовых реестрах. Сейчас запроса вроде нет, можно обойтись сортировкой.