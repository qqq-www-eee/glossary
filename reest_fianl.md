...
3. Если два человека имеют доступ к созданию реестров и преследуют при этом разные цели (ТН формирует реестр на выплату, а помощник прораба хочет распечатать работы какого-то мастера), то два и более реестра с одной и той же работой вынеждены сосуществовать одновременно. Однако сама работа обычно должна быть посчитана в лимитах единожды, поэтому система предоставляет возможность напрямую проставить нужным реестрам статус "Оплачено".
4. При просмотре некоторой работы система отобразит все реестры в статусе "Оплачено", куда вошла данная работа:
    1. в сайдбаре шахматки;
        1. При массовом выделении ячеек - в сайдбаре не выводятся никакие данные о реестрах.
    2. в карточке работы.
5. Добавить новый тип логов в историю изменений работы: "Задача внесена в реестр №ХХ".
6. К сдельным добавляются **почасовые** работы. Техническая реализация: в work_template добавляются нескольких новых почасовых работ с единицей измерения "час" из таблицы unit.  
7. Настройка спарвочника работ: при копировании работы из шаблона в шахматку помимо названия копируется также дефолтная расценка (из work_template.price в work.price).
    1. При создании реестра пользователь все так же сможет скорректировать расценку для текущего объекта (обновится work.price).
8. В карточку работы можно провалиться из боковой панели в профиле работы (а не только из профиля "Вид работ").
...
12. Еще нам нужна возможность поставить все-таки название реестру! Потому что можно создать много реестров для нескольких исполнителей, в которых будут отчеты конкретно по ним. У реестра должно быть корректируемое название!
