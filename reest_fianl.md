...
1. Необходимо добавить в шаблон почасовые работы:

- В таблице units уже есть единица измерения "час"

| id  | code | type | ru_display | ru_alias | base_unit | calculation_method |
| --- | ---- | ---- | ---------- | -------- | --------- | ------------------ |
| 26  | hour | time | ч          | [час, ч] | null      | number             |

- Осталось добавить в список работ следующие 9 работ:  
...
2. Добавить в шаблонные работы расценки согласно таблице: 

- Добавить в **work_template** поле **price**.
- Добавить в шаблонные работы расценки согласно таблице().

### Требования


При создании самого первого реестра цены не должны быть указаны? Да, должны, если они перетянуты из шаблона. У тех работ, что созданы руками, цены нет. По-хорошему сразу при создании работ надо указывать расценку, но мы редактируем через наряды. Так можно и цену скорректировать, и название работы не трогать.

1. Система дает возможность работать как с почасовыми, так и со сдельными работами.
2. В справочнике работ: работы из шаблона работ должны копироваться в список работ шахматки вместе с ценой (дефолтная цена из 1С). Копируем из work_template.price в work.price.
3. При создании реестра пользоватль может изменить цену конкретно для текущего объекта (обновится work.price, а work_template.price - нет).
4. Система должна показывать реестры в статусе оплачено также в карточке работы, не только в сайдпанели.
5. кстати надо будет добавить логов для истории изменений таски, типа "задачу внесли в реестр"
6. При массовом выделении ячеек - в сайдбаре НЕ выводятся данные о реестрах. Незачем выводить кучу реестров шахматки всех мастей.
7. В карточку работы должно быть можно провалиться из работы! А не только из профиля "Вид работ", очень не логично.
8. Более того, из "Вида работ" очень хочется провалиться в обычную работу, а не в карточку. А там уже есть переход в карточку и в логи.
...
3. Формирование нового реестра.
    ...
    2. Шаг 2 формирования нового реестра: Расценки
        ...
        3. Если расценка на работу с данным именем на данном объекте:
            1. ранее указывалась пользователем в этой шахматке ИЛИ изначально скопирована вместе с работой из шаблона, предзаполнить поле "Цена" этим значением (work.price);
            2. была создана пользователем и ранее не указывалась пользователем в этой шахматке () оставить поле "Цена" непредзаполненным.
