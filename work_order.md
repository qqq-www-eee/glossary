...
2. Даже если итерироваться по неделям, мы не знаем, по ПН у пользователя отчетность, по ПТ или еще когда-то. На одном и том же проекте сроки отчетности могут смениться.
3. Тажке мы не хотим случайно потерять аудиторию, которая по каким-то причинам работает 2х2, рассчитывается каждый день или еще по какому-то нестандартному графику.
4. Это подталкивает к тому, что рано или поздно **пользователь сможет выставлять даты отчетного периода произвольно**.  

> Такая свобода может привести к колизиям. Пример: коллега закрывает наряд за пн-ср, потом основной пользователь закрывает пн-пт и отправляет на оплату; позже коллега также отправляет свой наряд на оплату. В подобных случаях информация в проводках бухгалтерии имеет шанс задублироваться - и в целом столь неосмотрительное поведение может разочаровать пользователя.   
...
**Для поддержки частичной оплаты работ введем в БД структуру, поддерживающую вложенность "частичных работ" в "основные работы".** Для этого потребуется 4 таблицы, 2 из которых уже существуют, а 2 необходимо добавить, они очень простой структуры. Существуют payment_report, payment_report_task, task. Создать необходимо **subtask** и сопрягающую **subtask_payment_report_task**.  

> Можно было найти несколько верхнеуровневых подходов и реализовать задачу по-разному, например:
...
> Уже существующая таблица **payment_report_task** дополнительно подталкивала к этому.  

**Теперь надо решить 2 проблемы:** 
- В одной и той же шахматке создаются по каждому новому наряду множество "частичных работ", пересекающихся друг с другом по нескольким дням или (часто) вообще полностью дублирующие друг друга. Если работа за некий период времени могла попасть в разные наряды с разными статусами, то какой статус побеждает? Самый последний? Самый оплаченный? (ответ см. ниже)
- Если хоть один маленький кусочек работы в наряде уже был кем-то оплачен, что весь наряд идет насмарку, его надо пересоздавать (благо, это несложно). Лечится обучением пользователя, что созданный наряд желательно сразу же переводить в статус "Оплачено" - если не хотите, чтобы коллеги сделали ваш наряд устаревшим. Печать, подпись - все потом.

**Нам необходимо ввести и однозначно идентифицировать элементарную работу сквозным образом через всю систему** - нужна уникальная сущность для *работы в данный день в данном помещении*.
...
Сущность соответствует одному дню работы. Предлагаемые поля:
...
Одна элементарная работа может попасть во многие payment_report_task (и она будет это делать до тех пор, пока какой-нибудь из нарядов не перейдет с статус "Оплачен", проставив этой связанной элементарной работе статус is_paid = true).

### payment_report_task - частичная работа (таблица существует)

Сущность соответствует отдельной строке в наряде (оплаченного, не оплаченного, удалённого - все равно) и возникает сразу же при создании наряда, нужны в том числе для фильтрации и сортировки наряда и в целом являются неотъемлемой частью наряда.  
У частичных работ нет статусов - но можно в каком-то смысле их получить join'ами из нарядов (payment_order), от которых они созданы.  
Дату начала и дату конца расчетного перидоа для частичных работ придется также смотреть join'ами из родительского наряда.

В payment_report_task войдут те элементарные работы, которые:
- имеют такой же task_id, какой нас интересует исходя из текущего состояния шахматки;
- подходят по датам наряда,
- которые еще не были оплачены (is_paid = false).
...
Аналогичная история с тем, когда наряд теряет статус "Оплачен"~~(/"Отправлен на оплату")~~ (он становится "Удаленным" или "Не оплаченным").  
Тогда происходит апдейт статусов элементарных работ:
...
Приведем до кучи структуру **payment_report**, раз уж ее еще и обновления коснутся:

| Имя                    | Комментарий                                                                      | unique? | null?    | type         |
| ---------------------- | -------------------------------------------------------------------------------- | ------- | -------- | ------------ |
| payment_report_id      | Первичный ключ наряда                                                            | unique  | not null | uuid         |
...
| project_id             | uuid объекта (секции)                                                            |         |          | uuid         |
...
У частичных работ нет статусов - их надо смотреть join'ами от нарядов, в рамках которых они созданы (payment_order). payment_report_task нужны для фильтрации и сортировки при создании наряда и в целом необходимый логический промежуточный этап при формировании нарядов. Аналогичная история с датами расчетного периода.
...
Требования чисто по частичным оплатам:
