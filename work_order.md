...
Мы не хотим пользователя принуждать отчитываться строго по неделям - не больше и не меньше. Могут быть удобными расчеты раз в 2 недели, раз в месяц (кратно). И даже если итерироваться по неделям, мы не знаем, по понедельникам у клиента отчетность или по пятницам. Это все подталкивает к тому, что рано или поздно человек будет выбирать дату закрытия наряда и выставлять отчетный период **произвольно**. Это может привести к колизиям, когда мой коллега закрывает пн-ср, потом я закрываю пн-пт и отправляю на оплату. Затем коллега отправляет свой наряд на закрытие. Информация в проводках бухгалтерии имеет шанс задублироваться. Поэтому необходимо дополнительные проверки в системе все-таки производить.

Поэтому для "**Частичной оплаты работ**" необходима структура, поддерживающая вложенность "оплаченных частичных работ" в "основные работы" (/задачи/таски).  
...
> - Разделить сущности - "работа" и "частичная работа". Первая - "пустой контейнер", в котором всегда лежит как минимум одна частичная работа, вторая - основная сущность со всеми статусами "Ожидает дефектовку"/.../.../.../"Завершено"/"Готово к оплате"). Таска только собирает подзадачи вместе, обеспечивая читаемость процессов, а саб-таска все так же пересоздается каждую неделю с закрытием наряда.
...
Взаимосвязи **статуса оплаты** со **статусом выполнения** работ больше нет.  
...
- по каждому новому наряду (создаются как одним и тем же пользователем, так и разными) может создасться множество "частичных работ", которые отчасти пересекаются по одним и тем же дням, а могут (и часто будут) вообще полностью дублировать друг друга.
...
Тогда для каждого календарного дня и для каждой таски может существовать только одна сущность, называемая под-таской (элементарной работой).

Но сначала договрим про статус оплаты и статус прогресса работ. Они разделились:
