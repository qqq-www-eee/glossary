...
Мы не хотим принуждать пользователя отчитываться строго по неделям (ни больше и ни меньше!) - ему могут быть удобными расчеты раз в 2 недели, раз в месяц и пр. (кратные неделе периоды). Но даже если итерироваться по неделям, мы не знаем, по понедельникам у пользователя отчетность или по пятницам. Это подталкивает к тому, чтобы рано или поздно пользователь станет выставлять отчетный период **произвольно**.  
Это может привести к колизиям, например: коллега закрывает пн-ср, потом пользователь закрывает пн-пт и отправляет на оплату; затем коллега также отправляет свой наряд на оплату. В подобных случаях информация в проводках бухгалтерии имеет шанс задублироваться, да и такое поведение может просто разочаровать пользователя. Поэтому дополнительные проверки в системе предлагается все-таки производить.
...
Взаимосвязь **статуса оплаты** со **статусом выполнения** работ, таким образом, отсутствует.  
...
Она во многие payment_report_task попадет.
...
Теперь договорим про статус оплаты и статус прогресса работ. В итоге мы их разделяем:
