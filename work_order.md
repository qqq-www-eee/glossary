...
2. Однако даже если итерироваться по неделям, мы не знаем, по понедельникам у пользователя отчетность или по пятницам.
3. Тажке мы не хотим случайно потерять аудиторию, которая по каким-то причинам работает 2х2 или еще как-то.
4. Это подталкивает к тому, чтобы рано или поздно **пользователь сможет выставлять отчетный период произвольно**.  

Такая свобода, с другой стороны, может вести к колизиям. Пример: коллега закрывает пн-ср, потом пользователь закрывает пн-пт и отправляет на оплату; затем коллега также отправляет свой наряд на оплату. В подобных случаях информация в проводках бухгалтерии имеет шанс задублироваться, да и в целом такое поведение может разочаровать пользователя.  
Поэтому в системе предлагается все-таки проводить дополнительные проверки при формировании нарядов и при отображении оплаченных работ в шахматке.
...
Взаимосвязь **статуса оплаты** со **статусом выполнения** работ, таким образом, теперь отсутствует.  
Внутри каждого из статусов разрешены переходы из любого состояния в любое состояние. Переходы осуществляются пользователем вручную.

**Теперь надо решить проблему:** 
- большое число новых нарядов по одной и той же шахматке создаются как одним и тем же пользователем, так и разными. И по каждому такому новому наряду может создасться множество "частичных работ", которые отчасти пересекаются друг с другом по дням, а могут (и часто будут) вообще полностью дублировать друг друга. Если работа за некий период времени могла попасть в разные наряды с разными статусами, то какой статус будет у частичной работы? Самый последний? Самый оплаченный? (спойлер - самый оплаченный)
- даже если хоть один маленький кусочек работы в наряде уже был кем-то оплачен, что весь наряд идет насмарку, надо пересоздавать. Благо, это несложно. Желательно сразу же наряд распечатывать и вообще до кучи сразу переводить в статус "Оплачено" - это гарантирует, что ваши коллеги не сделают ваш наряд устаревшим.
...
**Тогда для каждого календарного дня и для каждой таски существует только одна элементарная работа.**

**SUBTASK - элементарная работа (новая таблица)**

Сущность соответствует одному дню работы (атомарная временная величина измерения работы). Предлагаемые поля:
...
Одна элементарная работа может попасть во многие payment_report_task. Будет попадать до тех пор, пока какой-нибудь наряд не перейдет с статус "Оплачен", проставив элементарной работе статус is_paid = true.

**payment_report_task - частичная работа (существующая таблица)**

Сущность соответствует одной строке какого-либо наряда (оплаченного, не оплаченного, удалённого).  
У частичных работ нет статусов - надо смотреть join'ами в наряды (payment_order), от которых они созданы.  
Зато у наряда есть статус. Если наряд получит статус "Оплачен", то все payment_report_task в его составе подразумеваются оплаченными. Далее надо понять про subtask - что там делать!!!

**payment_report_task нужны для фильтрации и сортировки при создании наряда и в целом необходимый логический промежуточный этап при формировании нарядов.**
...
ТУТ КСТАТИ ОБЯЗАТЕЛЬНО ДОЛЖНЫ БЫТЬ ДАТА НАЧАЛА И ДАТА КОНЦА РАСЧЕТНОГО ПЕРИОДА, ПОЗАИМСТВОВАННЫЕ ИЗ НАРЯДА. ВОТ ДОБАВИЛА ДВЕ ПОСЛЕДНИЕ СТРОКИ.
