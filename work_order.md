...
> - Расценки выставляются индивидуально для каждого мастера.
> - При выдаче доступа из пользователей надо убрать Имя и Должность - в них нет смысла, они все равно заменятся системными. Можно просто указать телефон. 
> - Необходимо реализовать "статусы наряда" с возможностью пользователю их менять ("Оплачен" / "Не оплачен"). 
> - Статус 'ReadytoPay' необходимо убрать из рабочих статусов тасок.
> - Для реализации частичной оплаты необходимо ввести сущность "подзадача" (видимо, на базе payment_report_task), у которой будет минимум два статуса - "Не оплачена" и "Оплачено" ~~(м.б. позже добавится "Отправлено на оплату")~~.   
> - Сабтаска создается при создании наряда.
> - По умолчанию статус сабтаски - "Не оплачена".
> - Если статус наряда меняется на "Оплачен", всем сабтаскам в его составе проставляется статус "оплачена"
> - Если статус наряда меняется на "Не оплачен", всем сабтаскам в его составе проставляется статус "не оплачена" ~~(возможно, их надо просто совсем стирать, надо обсудить)~~.
> - Так как пользователь должен перед созданием наряда сам все интересующие его работы перевести в статус "Завершено" - хорошо бы его этому обучить (напрямую подсказать).  
> - Необходимо обучение пользователя работе с фильтром по "сдельным" и "почасовым" работам.  
> - При формировании наряда надо сделать фильтр по сдельным и почасовым работам - то есть фильтр по единицам измерения работы.
...
> - В идеале, надо переименовать некоторые статусы тасок, т.к. немного нечитабельные и все разнородные, а некотооые устврели.
> - В шахматке закрашенным образом нам хотелось бы видеть **прогресс по таске целиком** (а не прогресс по объему работ, выданных текущему исполнителю на неделю). Поэтому в полях "Выдано работ" и "Выполнено" хотим видеть именно кумулятивну картину.
> - Пользователю надо объяснить! что вводить надо не задание на неделю, а цифру по всем исполнителям, корпевшим над работой.
...
> - Надо подумать, насколько сильно нас напрягает, что статус саб-тасок и нарядов, их породивших, всегда совпадает.
...
> - Если часть работ оплачена, но пользователь хочет уменьшить или уже уменьшил в боковой панели объем **выполненной работы** до значения, которое меньше объемов, за которые уже выплатили деньги:
>     - В момент сохранения данных боковой панели для помещения нужна нотификация с обратной связью (в духе "У вас сейчас случится перерасход или введено ошибочное значение для объема выполненных работ"), так как система стала неконсистентна - пользователю нужно либо скорректировать данные по помещению, либо перевести статус одного из оплаченных реестров в notPaid;
>     - рядом с полем "Выполнено" должен гореть знак, например, небольшой (!), подсвечивающий перерасход.
> - Если часть работ оплачена, но пользователь хочет уменьшить или уже уменьшил в боковой панели объем **дефектовки** до значения, которое меньше объемов, за которые уже выплатили деньги:
...
> Сразу приведем пример, с из-за которого все предлагается в таком виде, в каком предлагается
> 1. ПРИМЕР КОЛЛИЗИИ, с которой система должна успешно справиться: 
    - Пользователь закрыл наряд за 1-7 число месяца (статус наряда №1 "Оплачено). Пользователь закрыл наряд за 8-14 числа месяца (статус наряда №2 "Оплачено). Сегодня 22 число того же месяца. Пользователь закрывает наряд за 15-21 числа месяца (статус наряда №3 "Оплачено"), а потом переводит наряд №2 в статус "Сформирован" или в статус "Удален". Система должна не дать пользователю закрыть наряд за 8-21 число, ТОЛЬКО за 8-14! 
> 2. То есть, логично, **Из минусов - такой подход каждому новому наряду будет создавать груду подтасок.**
...
Одно из ключевых свойств наряда - его неизменность. Никакие его данные невозможно редактировать, кроме Статуса ("Сформирован", "Оплачен", "Удален") и Названия (по умолчанию "Наряд № [3] от [24 января 2023]"). В наряде нигде не используются ссылки на какие-либо сущности БД, потому что наряд - слепок определенного состояния шахматки за определенное время.  
В настоящее время удаление наряда невозможно ни из UI, ни напрямую из БД.  
...
## Требования к РАБОТА (отображение внутри нарядов)
...
> - Сейчас больше одного раза пользователь задачу в наряды положить не может;  
> - Сейчас в помещении больше одной работы данного типа быть не может (и не надо).

**Требуется**, чтобы в наряды попадали работы, обладающие свойствами:
1. находятся в рабочем статусе «Завершено»,
2. у них указаны Исполнители,
3. у них указаны выполненные объёмы
4. в **объемах** выполненной работы есть **изменения относительно предыдущих закрытых нарядов** ("закрытый наряд" = находятся в статусе  "Оплачен" ~~или "Отправлен на оплату"~~). Говорим здесь только про поле "Объем" (изменения в полях "Всего" (дефектовка) или "Остаток" (что останется на следующую неделю) или сколько "Выдано" исполнителю и пр. системе безразличны).

> - Многократное попадание работы в разные наряды позволит не пересоздавать работу в случае ее частичного выполнения и продолжить работу с той же сущностью.  
...
- Поддерживать две сущности - "работа" (пустой контейнер, в котором всегда лежит как минимум одна частичная работа) и "частичная работа" (основная сущность со всеми статусами "Ожидает дефектовку"/.../.../.../"Завершено"/"Готово к оплате"), тогда подзадачи - наша основная сущность, а таска только собирает их воедино, обеспечивая читаемость процессов;
- Поддерживать две сущности - "работа" и "частичная работа" (под-работа, саб-таска, sub-task), и **вложенной подтаске делегировать только статус об оплате**; все остальные свойства остаются с главной таской.  
...
| ~~'deleted'~~           | ~~'Удален' - статус частичной работы может повторять за статусом наряда для ясности, но прикладного смысла у этого нет. Будем отличать только черновики от оплаченных документов~~ |     
...
Атрибуты подзадачи сильно перекликаются с полями таблицы payment_report_task, но к ним, как минимум, необходимо добавить статус оплаты, чтобы, например, при отображении лимитов и перерасходов в боковой панели пользователя учитывать только реальные проводки, а не все, а также 
