...
Мы не хотим пользователя принуждать отчитываться строго по неделям (ни больше и ни меньше!), ведь могут быть удобными расчеты раз в 2 недели, раз в месяц и пр. (кратные неделе периоды). Но даже если итерироваться по неделям, мы не знаем, по понедельникам у пользователя отчетность или по пятницам. Это подталкивает к тому, чтобы рано или поздно пользователь выставлял отчетный период **произвольно**.
Это может привести к колизиям, например: мой коллега закрывает пн-ср, потом я закрываю пн-пт и отправляю на оплату; затем коллега также отправляет свой наряд на оплату. Информация в проводках бухгалтерии имеет существенный шанс задублироваться или просто разочаровать пользователя. Поэтому дополнительные проверки в системе предлагается все-таки производить.

Для "**Частичной оплаты работ**" введем структуру, поддерживающую вложенность "оплаченных частичных работ" в "основные работы" (/задачи/таски).  

> Это можно было бы реализовать учет по-разному, например:
...
> Уже существующая таблица **payment_report_task** дополнительно подталкивает к этому решению.  

Взаимосвязи **статуса оплаты** со **статусом выполнения** работ, таким образом, больше нет.  
Внутри каждого из статусов разрешены переходы из любого состояния в любое состояние, осуществляемые вручную пользователем.

**Теперь надо решить проблему:** 
- по каждому новому наряду (создаются как одним и тем же пользователем, так и разными) может создасться множество "частичных работ", которые отчасти пересекаются по одним и тем же дням, а могут (и часто будут) вообще полностью дублировать друг друга. И вот если частичная работа за некий период времени могла попасть в разные наряды с разными статусами, то какой статус будет у частичной работы? Самый последний? Самый оплаченный?  

**НАМ НЕОБХОДИМО ОДНОЗНАЧНО ИДЕНТИФИЦИРОВАТЬ элементарную РАБОТУ СКВОЗНЫМ ОБРАЗОМ ЧЕРЕЗ ВСЮ СИСТЕМУ** - нужна уникальная сущность для работы в данный день в данном помещении (проще говоря - для таски в данный день).

Будем считать, что меньше, чем один день, работа длиться не может.

Тогда для каждого календарного дня и для каждой таски может существовать только одна сущность, называемая элементарной работой (subtask).

**SUBTASK** - **элементарная работа** - **Новая таблица**

Эта сущность соответствует одному дню, когда исполнялась работа. Это атомарная временная величина измерения работы. Вот какие у нее будут поля:
...
**payment_report_task** - **частичная работа**

Эта сущность соответствует одной строке какого-либо оплаченного наряда. Таблица payment_report_task уже существует.  
У частичных работ нет статусов - их надо смотреть join'ами от нарядов, в рамках которых они созданы (payment_order).  
...
| Имя                         | Комментарий                                                                     | unique? | null? | type |
| --------------------------- | ------------------------------------------------------------------------------- | ------- | ----- | ---- |
| payment_report_task_id      | Первичный ключ                                                                  |         |       | uuid |
| work_name                   | Наименование работ, как было на момент формирования родительского наряда        |         |       |      |
| storey_number (storey_name) | Название или номер этажа как было на момент формирования наряда (строка)        |         |       |      |
| room_group_name             | Название или номер помещения как было на момент формирования наряда (строка)    |         |       |      |
| executor_user_full_name     | ФИО исполнителя как было на момент формирования наряда (строка)                 |         |       |      |
| volume                      | Объем работ, вычисленный для наряда                                             |         |       | uuid |
| total_price                 | Сумма к выплате для данного исполнителя по данному куску работы                 |         |       |      |
| task_id                     | uuid работы-таски, на базе которой создается частичная работа в наряд           |         |       |      |
| payment_report_id           | uuid родительского наряда                                                       |         |       |      |
| unit_id                     | uuid единицы измерения - оставляем id, потому что единицы измерения неизменяемы |         |       |      |

Теперь договорим про статус оплаты и статус прогресса работ. Они разделились:
