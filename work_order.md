...
У частичных работ нет статусов. Их надо смотреть join'ами от нарядов, в рамках которых они создавались (таблица payment_order).  Эти штуки нужны для удобной сортировки при создании наряда и ввобще как прромежуточный этап.
...
| 'paid'           | 'Оплачен' - пользователь переводит наряд в этот статус вручную. Все элементарные работы, до которых наряд может дотянуться, становятся со статусом 'is_paid' = true. При смене на любой другой статус из этого - все элементарные работы, до которых наряд может дотянуться, получают статус 'is_paid' = false. ~~При интеграйции с 1С факт оплаты может быть проставлен автомтачиеки.~~ | 
| 'deleted'        | 'Удален' - статус отчасти избыточный. Нужен только для front-end, с точки зрения пользователю пометить откровенный мусор (и то, непонятно, нужно ли). Прикладного смысла у данного статуса нет |  
