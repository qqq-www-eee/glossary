...
- Если хоть один маленький кусочек работы в наряде уже был кем-то оплачен, что весь наряд идет насмарку, его надо пересоздавать (благо, это несложно). Лечится обучением пользователя, что созданный наряд желательно сразу же переводить в статус "Оплачено" - если не хотите, чтобы коллеги сделали ваш наряд устаревшим. Печатать и подписывать - все потом.
...
Одна элементарная работа может попасть во многие payment_report_task - и будет попадать до тех пор, пока какой-нибудь из нарядов не перейдет с статус "Оплачен", проставив этой элементарной работе статус is_paid = true.

### payment_report_task - частичная работа (уже существует)

Сущность соответствует отдельной строке в наряде (оплаченного, не оплаченного, удалённого - все равно). 
Эти "частичные работы" создвозникают сразу же при создании "наряда".   
У частичных работ нет статусов - надо смотреть join'ами в наряды (payment_order), от которых они созданы.  

**payment_report_task** нужны в том числе для фильтрации и сортировки наряда и в целом являются неотъемлемой частью наряда.**

В каждой из payment_report_task войдут все элементарные работы:
- какие подходят по task_id,
- подходят по датам,
...
Тут до кучи нужна структура для **payment_report**:
