...
Где-то в списках нарядо в или в самом наряде (в статусах?) должна предоставляться возмножность "Удалить" наряд. Функционально между нарядом в статусе "Сформирован" и "Удален" нет никакой разницы, но отображение на фронте для пользователя может быть полезно. Либо давайте дадим возможность жестко удалять наряды.
...
- 1.1 **Статус выполнения** работ (**progress-status** у таски):
...
- 2.1 **Статус оплаты** (**payment-status**) для элементарной работы (**payment-status** сабтаски, поле 'is_paid'):  

| Значение поля is_paid | UI                                                                                          |
| --------------------- | ------------------------------------------------------------------------------------------- |
| true                  | Данная частичная работа была оплачена (определенное наименование работ в определенный день) |
| false                 | Данная частичная работа проведена, но не оплачена                                           |  
...
- 2.2 **Статус оплаты** нарядов payment_report (поле 'status'):
...
**Чтобы решить 2 проблемы:** 
- **такой подход по каждому новому наряду не создавал груду подтасок**
- **если под-таска на некий период времени не уникальна для нарядов (то есть один и тот же рабочий день вошел в виде копий в разные наряды) и у этих нарядов будут противоречащие статусы, то какой статус будет у саб-таски? Самый последний? Самый оплаченный?**
**НАМ НЕОБХОДИМО ОДНОЗНАЧНО ИДЕНТИФИЦИРОВАТЬ РАБОТУ СКВОЗНЫМ ОБРАЗОМ ЧЕРЕЗ ВСЮ СИСТЕМУ. Необходима уникальная сущность для работ.

Будем считать, что меньше, чем один день, работа длиться не может.

Тогда для каждого календарного дня и для каждого наименования работ может существовать только одна сущность, называемая под-таской. Вот какие у нее будут поля:
...
| ---------------------- | ------------------------------------------------------------ | ------- | -------- | ---- |
| subtask_id             | Ключ                                                         | unique  | not null | uuid |
| task_id                | Частью какой задачи (таски, работы) является подзадача?      | -       | not null | uuid |
