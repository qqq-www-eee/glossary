...
- находятся в рабочем статусе «Завершено»,
- у них указаны исполнители,
- у них указаны выполненные объёмы,
- они не попали в сформированные ранее ~~табели~~ наряды.

То есть сейчас больше одного раза пользователь задачу в наряды положить не может.  
При этом в помещении больше одной работы данного типа быть тоже у нас (пока) не может (и не надо!).

Куда мы хотим прийти:
- находятся в рабочем статусе «Завершено»,
- у них указаны исполнители,
- у них указаны выполненные объёмы
- в этих объемах что-то изменилось по сравнению с предыдущими нарядами, которые отправлены на оплату или оплачены.
- А почему, если дефектовка не заполнена, то у нас все окей получается? Что важнее - выдано или дефектовка?

Если у нас перерасход. Почему это в закрытие наряда не должа попасть работа, которая уже на 120% сделана? Она должна попасть, если новый объем работ у нас больше, чем объем работ в последнем созданном наряде,  
Если работа находится в статусе выполнения «Завершено», это не значит, что потом нельзя ее перевести в другой статус. Можно, еще как, в любой.  
Но при создании наряда надо таки все интересующие нас работы перевести именно в статус "Завершено".  

Предлагается вести в системе ДВА статуса (у двух разных как бы сущностей)!
Первый относится к **работе**,  
Второй - к сущности **оплаченный работы кусок**:  

- **Статус выполнения** работ ("progress-status"):
...
- **Статус оплаты** работ ("payment-status"):
...
При этом перевод статусов оплаты должен быть строгим! То есть смене статуса руками дожна вылезать модалка с подтерждением (или должен быть оч. строгий сопровождающий тектст нужен - мол работы этого наряда будут учитываться как получившие полную или частичную оплату! А если отменяем статус оплачено: работы этого наряда больше не будут считаться оплаченными, что повлечет мол за собой недоразумения и коллизии различные в шахматке и боковой панели).

### Поля работ, пожалуйста

1. фва
2. фыва

1. ID для UI
2. Авторский комментарий к наряду
3. Дата начала периода
4. Дата окончания периода
5. 
...
На шаге 1 формирования наряда должна не только отразаться информация - за какой диапазон дат (и сколько в итоге дней накапало), какая секция, кто создает, но и дать возможность ввести комментарий - то есть как-то обозвать наряд - имя/коммент. Подтянуть ссылку на предыдущий наряд ушедший на оплату по этому проекту.

<img width="1000" alt="new order" src="Frame 47.png">

На шаге 2 обращаем внимание, что договоренности по оплате с разными работниками разные. А также что могут быть работы почасовые - но мы их накак, кроме как у себя в голове не различаем.
...
Есть два предложения по БД, одно из них тупо сделать две сущности - одно работа и другое частичная работ, и вот работа - это пустой контейнер, в котором всегда лежит как минимум одна частичная работа. У частичной работы уже есть статусы: "Ожидает дефектовку"/.../.../.../"Завершено"/"Готово к оплате".
...
Третий вариант - создать таблицу частичных оплат, в которую просто захоидить и проверять, были ли по данной работе транзакции. Предлагаю пойти по третьему пути. 


Так вот есть два принципиальных пути - статус об оплаченности может быть у куска работы или 
...
При оплате с таской ничего не происходит. Таску на остаток создать нельзя - потому что:
- у нас ограничений на поля боковой панели никаких нет: их можно в любой момент редактировать. Значит, нет соответствия (вычисляемых ограничений) между текущим уровнем прогресса по работам, значением декектовки ("Всего"), суммарными начислениями за проделанную работу, лимитами. Как собирать таски на остаток?
- может быть перерасход
...
Поскольку "Оплаченность" важна именно в разрезе какой-то определенной работы или части этой работы, связь с реестрами на выплату должна быть железная! То есть реестр - это отписка, а основная сущность - это наряд с детализацией.
...
Это вообще бог его знает что такое, оно на самом деле - просто расширенное представление текущей информации о шахматке... С сылками на все актуальные работы... ну пусть его. Только отчеты все-таки за какой-то период предполагаются. а тут что-то вроде развертки шахматки по разным измерениям данных.
...
1. **Номер наряда** для UX - натуральное число, отображаемое для удобства пользователя;
2. **Дата-время создания** наряда;
3. Поле "За период" - **стартовая дата-время**;
4. Поле "За период" - **дата-время окончания периода**;
4. **ФИО создателя** - как он звался на момент создания наряда (у себя в учетной записи, но не как в доступах у хозяина шахматки, сейчас это совпадает, тем более, все это барахло уберем).
5. **Должность создателя** - как было на момент создания наряда (у автора в учетной записи, но не как у хозяина шахматки, тем более все это барахло уберем). 
5. **Статус** - "Сформирован" / "Оплачен". Удалять отчеты на текущий момент нельзя. Оплаченные наряды отображаются в боковой панели. На крайний случай - у системы есть откат, пользователь может вернуться и заново создать наряд, отчет, реестра на выплату - это синонимы.
4. ~~По типу работ (сдельная-почасовая) мы пользователя не ограничиваем. Если хочет, пусть миксует их прям в нарядах, но постараемся научить их выставлять фильтры, чтобы наряды получались раздельными.~~
6. Комментарий к наряду - что-то вроде имени.

Итого 8 элементарных полей вроде бы
