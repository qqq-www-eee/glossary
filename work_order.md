...
> В системе предлагается осуществлять дополнительные проверки при формировании нарядов и при отображении оплаченных работ в боковой панели в шахматке.
...
- В одной и той же шахматке создаются по каждому новому наряду множество "частичных работ", пересекающихся друг с другом по нескольким дням или (часто) вообще полностью дублирующие друг друга. Если частичная работа за некий период времени могла попасть в разные наряды с разными статусами, то какой статус побеждает? Самый последний? Самый оплаченный? (спойлер - самый оплаченный, см. ниже)
- Если хоть один маленький кусочек работы в наряде уже был кем-то оплачен, что весь наряд идет насмарку, его надо пересоздавать (благо, это несложно). Лечится это обучением пользователя - созданный наряд желательно сразу же переводить в статус "Оплачено", если не хотите, чтобы коллеги сделали ваш наряд устаревшим. Печать, подпись - все потом.

В дальнейшем обсуждении будут задействованы 5 таблиц, 3 из которых уже существуют, а 2 необходимо добавить, и они очень простой структуры.  
Существуют: **payment_report**, **task**, **payment_report_task**.  
...
Сущность соответствует отдельной строке в наряде (оплаченного, не оплаченного, удалённого - все равно) и возникает при создании наряда. Нужна в том числе для фильтрации и сортировки работ в наряде и в целом является неотъемлемой частью наряда, его структурной частью.  

> У частичных работ нет статусов - в каком-то смысле их можно получить join'ами из нарядов (payment_order), от которых они созданы.  
> Дату начала и дату окончания расчетного периода для частичных работ также придется доставать join'ами из родительского наряда.
...
set is_paid = true
where 
  (select с выбором всех тасок, что связаны с payment_report_task, которые относятся к payment_report, получившему статус "Оплачен")
returning *;
```
...
Тогда происходит апдейт статусов элементарных работ в сторону "Не оплачен" (и теперь они будут попадать во все новые наряды).
...
В общем-то смысл всего происходящего вкратце - в боковой панели шахматки отображать все оплаты по данной работе просто FYI, пользователь.
