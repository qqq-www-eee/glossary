...
3. Тажке мы не хотим случайно потерять аудиторию, которая по каким-то причинам работает 2х2, рассчитывается каждый день или использует еще какой-то нестандартный график.
...
> Поэтому в системе предлагается осуществлять дополнительные проверки при формировании нарядов и при отображении оплаченных работ в боковой панели в шахматке.

**Для поддержки частичной оплаты работ введем в БД структуру, поддерживающую вложенность "частичных работ" в "основные работы".**
...
> Структура уже существующей таблицы **payment_report_task** дополнительно подталкивала к этому.    

**Теперь остается решить еще 2 проблемы:** 
...
Далее будут задействованы 5 таблиц, 3 из которых уже существуют, а 2 необходимо добавить, и они очень простой структуры.  
Существуют: **payment_report**, **payment_report_task**, **task**.  
Создать необходимо **subtask** и сопрягающую **subtask_payment_report_task**.
