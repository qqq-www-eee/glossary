...
1. Мы не принуждаем пользователя отчитываться строго по неделям. Ему могут быть удобны расчеты раз в 2 недели, раз в месяц и другие кратные неделе периоды.
2. Даже если итерироваться по неделям, мы не знаем, по ПН у пользователя отчетность или по ПТ.
3. Тажке мы не хотим случайно потерять аудиторию, которая по каким-то причинам работает 2х2 или еще по какому-то нестандартному графику.
4. Это подталкивает к тому, чтобы рано или поздно **пользователь сможет выставлять отчетный период произвольно**.  

> Такая свобода может привести к колизиям. Пример: коллега закрывает пн-ср, потом пользователь закрывает пн-пт и отправляет на оплату; затем коллега также отправляет свой наряд на оплату. В подобных случаях информация в проводках бухгалтерии имеет шанс задублироваться, да и в целом такое неосмотрительное поведение может разочаровать пользователя.   
> Поэтому в системе предлагается осуществлять дополнительные проверки при формировании нарядов и при отображении оплаченных работ в шахматке.

**Для "Частичной оплаты работ" введем в БД структуру, поддерживающую вложенность "оплаченных частичных работ" в "основные работы" (/задачи/таски).**  

> Это можно было бы реализовать по-разному, например:
> - Каждую неделю все текущие таски закрываются и пересоздаются таски на остаток - но тогда раз в неделю, фактически, вся шахматка будет целиком принудительно обновляться (будет сложно версионировать? сложно читать логи? сложно проводить аналитику? зато не нужно вводить новые сущности).
...
> - Разделить сущности - "работа" и "частичная работа" (/подработа/саб-таска/sub-task), причем **вложенной подтаске делегировать только статус оплаты**, оставив остальные свойства за основной таской.  
> 
> **Пойдем по третьему пути**.  
> Уже существующая таблица **payment_report_task** дополнительно подталкивает к этому.  

Взаимосвязь **статуса оплаты** со **статусом выполнения** работ, таким образом, теперь будет отсутствовать.  
...
**Теперь надо решить 2 проблемы:** 
- в одной и той же шахматке создаются по каждому новому наряду может создасться множество "частичных работ", отчасти пересекающихся друг с другом по нескольким дням, а могут (и часто будут) вообще полностью дублировать друг друга. Если работа за некий период времени могла попасть в разные наряды с разными статусами, то какой статус у нее будет? Самый последний? Самый оплаченный? (см. ниже)
- если хоть один маленький кусочек работы в наряде уже был кем-то оплачен, что весь наряд идет насмарку, надо пересоздавать (благо, это несложно). Лечится тем, что созданный наряд желательно сразу же переводить в статус "Оплачено" - это гарантирует, что коллеги не сделают ваш наряд устаревшим. Печатать и подписывать потом.

**Необходимо однозначно идентифицировать элементарную работу сквозным образом через всю систему**. Нужна уникальная сущность для работы в данный день в данном помещении.
...
Сущность соответствует одному дню работы (атомарная работа). Предлагаемые поля:
...
Одна элементарная работа может попасть во многие payment_report_task. Будет попадать до тех пор, пока какой-нибудь наряд не перейдет с статус "Оплачен", проставив этой элементарной работе статус is_paid = true.

### payment_report_task - частичная работа (существует)

Сущность соответствует отдельной строке какого-либо наряда (оплаченного, не оплаченного, удалённого, все равно). 
Эти "частичные работы" создаются при создании "наряда". 
У частичных работ нет статусов - надо смотреть join'ами в наряды (payment_order), от которых они созданы.  
Зато у наряда есть статус. Если наряд получит статус "Оплачен", то все payment_report_task в его составе подразумеваются оплаченными.

**payment_report_task нужны для фильтрации и сортировки при создании наряда и в целом являются необходимым логический промежуточным этапом формирования нарядов.**

Эти сущности создаются каждый раз при создании нового наряда. В каждой из payment_report_task войдут все элементарные работы, какие подходят по task_id и по датам, которые еще не были оплачены! (is_paid = false).
...
| report_start_date           | Начало расчетного периода                                                       | -       | not null | datetime     |
| report_finish_date          | Конец расчетного периода                                                        | -       | not null | datetime     |

 Две последние строки - ДАТА НАЧАЛА И ДАТА КОНЦА РАСЧЕТНОГО ПЕРИОДА, позаимствованные из родительского наряда. Надо понять, нужны ли они или тоже джоинами посмотреть можно.

У payment_report_task И subtask соотношение многие-ко-многим. Значит, потребуется таблица для их соединения типа:

**subtask_payment_report_task**


| Имя                    | Комментарий                       | unique? | null?    | type |
| ---------------------- | --------------------------------- | ------- | -------- | ---- |
| sprt_id                | Первичный ключ связи              | unique  | not null | uuid |
...
Когда наряд получает статус "Оплачен" происходит:
...
то есть всем subtask (элементарным работам) проставляется статус, мол они оплачены. А всем нарядам, которые задействуют subtask'и со свежепроставленными статусами "Оплачена" меняют статус на "Удален". Потому что он стал уже недействительным.

Аналогичная история с тем, когда наряд теряет статус "Оплачен"~~(/"Отправлен на оплату")~~ (он становится "Удаленным" или "Не оплаченным"). Тогда выполняется:
...
то бишь всем элементарным работам, которые были оплачены, а теперь послетали, проставляется статус неоплачена, и теперь они будут попадать во все новые наряды.

<br>

<br>

Теперь договорим про статус оплаты и статус прогресса работ. Вот как мы их в итоге разделяем:
...
У частичных работ нет статусов - их надо смотреть join'ами от нарядов, в рамках которых они созданы (payment_order). payment_report_task нужны для фильтрации и сортировки при создании наряда и в целом необходимый логический промежуточный этап при формировании нарядов.

ТУТ КСТАТИ ОБЯЗАТЕЛЬНО ДОЛЖНЫ БЫТЬ ДАТА НАЧАЛА И ДАТА КОНЦА РАСЧЕТНОГО ПЕРИОДА, ПОЗАИМСТВОВАННЫЕ ИЗ НАРЯДА.
...
| 'deleted'        | 'Удален' - статус отчасти избыточный. Нужен только для front-end, чтобы пользователю отметить откровенно не нужные ему наряды, черновики (нужно ли?). |  
