...
По бизнес-смыслу предполагается, что каджая работа в конце своего жизненного цикла должна быть закончена и оплачена, другого пути у нее нет. Она может делаться по частям (более одной недели), даже каждую неделю новыми исполнителями, но в итоге все они в сумме должны будт покрыть тот объем, который изначально диктуется дефектовкой.

Работы бывают сдельными (их подавляющее большинство) и почасовыми. Почасовые работы, как правило, представлены разнообразными мелкими доработками. 

Процесс частичной оплаты труда иногда называют процентовкой, если мы .


Сдельщики и часовщики технически все используются в тасках, отчетах, нарядах, реестрах одинаково.
Требование бизнеса "схлопывать не стоит" реализуется через выставление правильных фильтров.
Имеем в виду, что часовщики не всегда есть
Важно знать что закрывалось по часам, а что сдельно - работы у нас просто отдельные.
...
4. ФИО создателя и должность создателя как он звался на момент создания наряда. Ведь наряд штука неизменяемая.
...
6. Комментарий к наряду - вместо изменяемого имени.
