...
- Каждую неделю все текущие таски закрываются и, если требуется, пересодаются таски на остаток - тогда раз в неделю, фактически, вся шахматка целиком принудительно обновляется (будет сложно версионировать? сложно читать логи? сложно проводить аналитику? зато не нужно вводить новые сущности);
- Разделить сущности - "работа" ("пустой контейнер", в котором всегда лежит как минимум одна частичная работа) и "частичная работа" (основная сущность со всеми статусами "Ожидает дефектовку"/.../.../.../"Завершено"/"Готово к оплате"), тогда подзадачи - основная сущность, а таска только собирает их вместе, обеспечивая читаемость процессов;
- Разделить сущности - "работа" и "частичная работа" (/подработа/саб-таска/sub-task), и **вложенной подтаске делегировать только статус об оплате**; остальные свойства остаются с основной таской.  

**Предлагается пойти по третьему пути**. Структура с таблицей **payment_report_task** дополнительно подталкивает к этому решению.  

Взаимосвязи **статуса оплаты** со с**татусом выполнения** работ больше не будет.  
Внутри каждого из статусов разрешены переходы из любого состояния в любое состояние (осуществляются вручную пользователем).

Статус оплаты и статус прогресса работ разделятся:
...
- **Статус оплаты** (**payment-status**) элементарной работы (**payment-status** у сабтаски, поле 'is_paid'):  

| Значение поля is_paid | UI                                                                                     |
| --------------------- | -------------------------------------------------------------------------------------- |
| true                  | Данная частичная работа оплачена (определенное наименование работ в определенный день) |
| false                 | Данная частичная работа проведена, но не оплачена                                      |  

- **Статус оплаты** (**payment-status**) частичных работ payment_report_task:
...
и нарядов payment_report (значения у них совпадают, поле называется 'status'):
