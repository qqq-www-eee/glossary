...
> - При выдаче доступа из пользователей надо убрать Имя и Должность - в них нет смысла, они все равно заменятся системными, можно просто указывать только телефон. 
> - Необходимо реализовать "Статус наряда" с возможностью для пользователя их менять ("Сформирован" / "Оплачен" / "Удален"~~/ м.б. позже добавится "Отправлено на оплату"~~). 
> - Статус 'ReadytoPay' необходимо убрать из рабочих статусов тасок.
> - Для реализации частичной оплаты необходимо ввести сущность "подзадача" / sub-task (видимо, на базе payment_report_task), у которой будет минимум два статуса - "Не оплачена" и "Оплачена".   
> - Подзадача, или сабтаска, заводится при создании наряда???
