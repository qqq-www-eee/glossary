...
5. Фильтрация или Быстрый поиск в списке нарядов отсутствует. По-хорошему нужна поддержка поиска реестров, содержащих исполнителя, название работ, место проведения - так же, как это делается при создании нового наряда.
6. Возможность закрыть работу руками, чтобы она больше в реестры не попадала.
7. Чтобы не терять наполнение работ исполнителями надо дать возможность редактировать "контакты" пользователя. 
8. Должен быть поиск по списку реестров, помогающий пользователю идентифицировать нужный реестр за определенный расчетный период (все такие реестры.)
