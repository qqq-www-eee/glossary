...
17. ~~Функционал закрытия реестров будет необходим в мобильном приложении как только будет реализована интеграция с 1C. Он будет работать только если устройство онлайн.~~
...
1. "Частичных оплат" более нет (Частичные оплаты и полные оплаты не различимы):
    ...
    2. Работы нельзя "закрыть" какой-либо оплатой;
    3. Работа может получить сколь угодно много отметок об оплате попав в несколько разных нарядов;
    4. При формировании реестра новые записи о работах "на остаток" не создаются;
    5. Общее число работ в проекте в стандартном случае постепенно становится равно **числу_этажей * число_комнат * число_названий_работ** - и далее больше не растет.
2. Реестры на оплату сближаются по функциональности с отчетами. Однако отчеты и реестры отображают разные поля в разном порядке - то есть несмотря на схожесть подачи информации, предназначение у них разное (у отчетов - финансовое и операционное планирование, у нарядов - корректно подсчитать текущие расходы на выплаты).
...
5. Реестр на текущий момент невозможно жестко удалить.
6. У реестра возможно вручную менять статус:
    1. "Сформирован" - нейтральный статус, дается по умолчанию при создании реестра, реестр отображается в списке реестров;
    2. "Оплачен" - обозначает, что реестр должен учитываться системой при работе с лимитами (в боковой панели шахматки);
    2. "Удален" -  обозначает, что реестр непригоден или не интересен для дальнейшей работы; реестр не отображается в списке реестров, но может быть просмотрен пользователем и возвращен к статусу "Сформирован" или "Оплачен";
