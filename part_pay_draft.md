...
8. ~~**Частичный перерасход** - состояние частичной работы, при котором текущий объем выполненных работ превышает значение поля "Выдано". Это скорее не перерасход, а это скорее ошибка - зачесть работы и не учесть, куда взяты материалы - ошибка в веденных данных. Пока есть смысл никак на этом не фокусироваться.~~

### Некоторые тезисы

1. Для описания частичного закрытия нарядов недостаточно одной таски. Таски необходимо расслоить на две сущности с более понятными атрибутами и статусной моделью. В частности:
    1. "Ожидает дефектовку" и "Готово к работе" - непонятно чем отличаются.
    2. Более того, по сути различаются статусы "Ожидает дефектовку" и "Ожидает дефектовку" - для созданной и несозданной таски. Сначала "Ожидает дефектовку" = "Таски нет, статуса нет", если таска не создана, а если создана (например была указана дефектовка = 0) то это становится реальным статусом задачи "Ожидает дефектовку".
