...
1. Для описания частичного закрытия нарядов оказалось недостаточно понятия таски. Наши таски необходимо разделить на две сущности с более понятными атрибутами и статусной моделью. Что привело к такой мысли:
    ...
    9. **Итог: "статусы рабочего процесса" прикрепить к работе, не к частичной работе (у нас это так). "статус оплаты" при этом невозможно отделить от частичной работы (ей нужно делегировать поле paid_at). Плюс работам добавить как минимум 1 новый статус вроде readyToPay**.
2. За частичную работу платят в рамках одного отдельно взятого расчетного периода (и наряда). После закрытия этого наряда для вошедших в него частичных работ, **если их основная работа еще себя не исчерпала, пересоздаются новые сущности частичных работ "на остаток"** - чтобы уже оплаченные работы больше ни в один наряд не попали и работа при этом не затерялась. Но что значит "основная работа себя не исчерпала"?
    1. Когда **перестаем** создавать таски на остаток? (система должна знать, когда ей остановиться, иначе в конце каждой работы будут вечно висящие таски, попадающие в наряды):
        1. opt1: Хотя значение дефектовки можно редактировать, при пересоздании частичных работ возможно полагаться на это поле (тогда Пользователю необходимо подстветить и объяснить эту логику - свободный ввод данных в поле дефектовка не значит, что мы на нее не смотрим - мы смотрим). Но пользователь может захотеть работать в шахматке с ситуацией с контролируемым перерасходом - придется костылить: вводить в дефектовке ненастоящие значения, все равно потребуется кнопка для создания таски, придется откатывать до предыдущей версии, правя дефектовку там и пр. Очень тяжелый вариант, перегруженная логика.
        2. opt2: Всегда пересоздавать таску на остаток, но дать пользователю прямую кнопку, как убить таску.
        3. opt3: Новый статус: пользователь переводит работы в статус "Готово1" (readyToPay), при котором таска будет пересоздаваться на остаток, или в другой статус "Готово2" (completed), при котором таска на остаток пересоздаваться уже не будет. Первая на вкладке "Обзор" покрасит фасолину, скажем, в сине-зеленый; вторая - в ярко-зеленый.
            ...
            2. Если объем выполненных работ совпадает с дефектовкой или превышает ее, то пользователю необходимо подсказать, что ему, вероятно, нужен статус 'completed', когда он пытается проставить 'readyToPay'. Возможно, это надо делать в боковой панели, а возможно - уже в нарядах.
        4. opt4: Можно при формировании наряда прямо спрашивать - вы эту работу хотите дальше отслеживать? Надо будет все время проставлять либо тогглы, либо чекбоксы. Немного не дружественно, но от проставления статусов всем записям о работе недалеко ушло.
    2. А каковы вообще базовые условия попадания частичной работы в наряд?
        1. Правильный статус:
    ...
    4. Еще один раз о том же. С перспективы того, что есть на данный момент: кашу из статусов надо разделить по 3-м сущностям: "помещение", "родительская работа" и "частичная работа".
        ...
        2. Распределение:
            ...
            2. Работе достаются: "В работе", "Ожидает технадзор", "Есть замечания", "Завершено". Менее очевидно, но также достаются работе "Готово к оплате" и "Ожидает дефектовку".
