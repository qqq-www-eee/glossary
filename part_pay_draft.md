...
3. Формирование нового реестра.
    1. Шаг 1 формирования нового реестра: Работы
        ...
        4. Система должна отображать для каждой работы ее текущий статус ("Завершено", "Ожидает дефектовку") ~~(возможно отображать в виде светофора)~~.
    ...
    2. Шаг 2 формирования нового реестра: Расценки
        ...
        4. Порядок отображения полей - алфавитный
        5. Фильтрация и сортировка
        6. Система позволяет пользователю вернуться на экран выбора работ для наряда без сохранения значений полей "Цена", даже если они были заполнены пользователем.
        7. Если хотя бы для одной работы поле "Цена" не заполнено, система отрисовывает ошибку для этих инпутов и не переходит далее.
...
5. Отображение **записей об оплате** в **боковой панели**.
    1. В интерфейсе шахматки: если в **левой панели** выбрана некоторая работа и в **рабочей области** выбрана некоторая ячейка/ячейки, в **боковой панели** система отображает данных по всем **записям об оплате** (payment_report_task) для данной комбинации работы + этаж + room_group, если только связанный с ней наряд имеет статус "Оплачено". Отображаемые данные:
        1. ~~Номер наряда~~ (если помещается);
        2. Дата наряда без времени;
        3. Исполнитель;
        4. Объем;
        5. ~~Расценка~~ (если помещается);
        6. ~~К выплате~~ (если помещается).
    2. Порядок сортировки записей об оплате - хронологический. Самые недавние записи раполагаются выше, самые старые - ниже.
    3. Данные кликабельны. По клику на строку можно перейти к конкретному наряду.
