...
1. Чаще всего, расчетный период = неделе.
2. "Дефектовка это вообще история, которую делают в самом начале объекта, когда принимают его" Дефектовка вообще происходит на объекте только один раз, в самом начале самой первой работы. При этом новые замечания технадзоры оставляют каждый расчетный период. Налицо явная нестыковка в статусах, еще не касаемо частичных оплат (статусов об оплате). Нам недостаточно для описания частичных работ одной только таски, неудобно, почти невозможно. Становятся все более очевидными требования к статусам. В частности, "Ожидает дефектовку" и "Готово к работе" - непонятно чем отличаются. Еще три серых цвета, конечно, вопросы вызывают.
...
4. За частичную работу платят в рамках одного отдельно взятого расчетного периода (и наряда). После закрытия этого наряда для вошедших в него частичных работ, **если основная работа еще себя не исчерпала**, пересоздадутся новые сущности частичных работ "на остаток" - чтобы уже оплаченные работы больше ни в один наряд не попали. Хотя дефектовка - фиг знает какое значение, его можно редактировать, но пересоздание тасок полагается именно на это поле.
5. **Дефектиовка - это характеристика помещения**. Как и тип помещения, который мы локально впихнули в название. Дефектовку можно корректировать, но в базе она не там лежит.
6. Работа имеет статусы ~~"Ожидает дефектовку"~~/"Готово к работе"/"В работе"/"Ожидает технадзор"/"Есть замечания"/"Завершено" и др., никаких дефектовок и тем более статусов об оплате! 
7. Ключевой момент - разнести "статус рабочего процесса" и "статус оплаты", потому что они попросту независимы. Вторая сущности нужна для нарядов внутри, первая - для вкладки "Обзор".
8. Считаем, что дата создания наряда совпадает с датой окончания расчетного периода.
...
Короче, еще раз. Эту кашу из статусов надо распихать по трем сущностям: помещение, родительская работа, частичная работа.
...
Квартире достается ничего.
Дефектовка относится к каждому типу работ + помещение? это для родительской таски. Ожидает дефектовку и отдефектовано = готов к работе. 
Частичной работе достается - WIP, ожидает технадзор, есть замечания, завершено (=готово к оплате), оплачено.

Значит, сначала создается шахматка, в ней вообще нет работ. 
Выходим на вкладку обзор - и там у нас пустые фасолины (например 5 шт) и подписана с какого-то перепугу первая работ в этой группе, видимо, идущая чисто по алфавиту. Вовсе не по бизнес-логике! Это важно. Зачем мы показываем рандомную работу?

Число фасолин вычисляет почему-то на основе работ, заполненных в шахматке. То есть если в виде работ у нас 6 наименований работ, то 6 будут только если заполнены все 6 из них хоть где-то.
