...
1. Для описания частичного закрытия нарядов оказалось недостаточно понятия таски. Наши таски необходимо разделить на две сущности с более понятными атрибутами и статусной моделью. Что привело к такой мысли:
    ...
    3. Три серых цвета ни при чем, но хочется их здесь отметить.
    4. Дефектовка - это история, которую делают один-единственный раз в самом начале объекта, когда принимают его. Из-за передачи фронта работ дефектовка ближе к атрибутам "работы", чем к атрибутам "помещения", но она точно не относится к сущности, оседающей в закрываемых нарядах. 
    5. При этом новые замечания технадзоры оставляют каждый расчетный период.
    6. Статусы об оплате, какими бы они ни были, также должны работать с периодичность в неделю, но главное - тем сущностям, которые попадают в частичные наряды и в рамках них же закрываются, нужен некий флаг ("статус", либо просто bool, либо дата оплаты), отличающий уже оплаченную порцию работы от пока неоплаченной .
    7. Ячейка "Вид работ" нужна зеленой тогда, когда действительно закончен целый срез работ. То есть зеленая фасолина должна обозначать одну реально сделанную работу. Даже если мы хотим, чтобы ячейка "Вид работ" красилась каждую неделю в знак завершенных частичных работ, нам лучше эти два состояния отличать - например, красить фасолину в сине-зеленый и отдельно в ярко-зеленый. Это позволит пользователю отличать во вкладке обзор работы, которые "Готовы в рамках периода" от тех, что "Совсем готовы". 
    8. Как понять, что частичная работа готова к оплате? За счет нового промежуточного статуса - это самое просто решение.
    9. **Итог - "Статусы работчего процесса" крепить к работе, не к частичной работе (у нас так). "Оплату" при этом никак невозможно отделить от частичной оплаты, ей нужно делегировать поле paid_at. Плюс работам добавить как минимум 1 новый статус**.
2. За частичную работу платят в рамках одного отдельно взятого расчетного периода (и наряда). После закрытия этого наряда для вошедших в него частичных работ, **если их основная работа еще себя не исчерпала, пересоздаются новые сущности частичных работ "на остаток"** - чтобы уже оплаченные работы больше ни в один наряд не попали и работа при этом не затерялась. Но что же такое "основная работа себя не исчерпала"? Также система должна знать, когда ей остановиться, иначе в конце каждой работы будут вечно висящие таски, попадающие в наряды.
    1. Какие частичные работы попадают в наряд?
        1. Те, что находятся в правильном статусе:
    ...
    3. Не смущает ли нас, что фасолины красятся в зеленый и также ячейка "Видов работ" красится в зеленый по одному принципу (статус), а вот на оплату уходят другие (их подмножество, у которых указаны объемы и указаны исполнители)?   
    4. Еще один раз об одном и том же с перспективы того, что есть на данный момент: кашу из статусов надо разделить по 3-м сущностям: "помещение", "родительская работа" и "частичная работа".
        1. Полный список:
