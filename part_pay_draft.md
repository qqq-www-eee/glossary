...
1. **Наряд** - таблица, отражающая объёмы выполненных работ (сдельных, почасовых) за определенный период (часто за неделю) по всем работникам секции. Наряды закрывются технадзорами.
2. **Реестр на выплату** - документ, формируемый по закрытии наряда руководителем координации. Это финансовая выжимка наряда с указанием сумм к выплате, отправляемая в бухгалтерию. 
3. **Расчетный период** - период времени, за который составляется наряд.
4. **Работа** - проведение определенного наименования работ в определенном помещении. Работа разбивается на таски, у которых исполнитель подразумевается по бизнесу неизменным за ее время жизни.  
5. **Частичное закрытие наряда** (иногда **процентовка**) - процесс подготовки документов для частичной оплаты за одну и ту же работу. Закрытие таски, закрытие частие частиной работы, закрытие задачи.
7. То, что мы сейчас зовем таской, содержит информацию о работе, за которую хотят заплатить в рамках одного отдельно взятого наряда. После чего пересоздастся такая же сущность "на остаток" - чтобы уже оплаченные работы больше ни в один наряд не попали.
8. И должна быть другая сущность типа "работа", "глобальная таска", "задача", task, что-такое. Она определяется по комбинации **work_id** + **storey** + **room_group** и по сути есть слайс помещения под конкретную работу. Имеет статусы "Дефектовка"/"В работе"/"Замечания"/"Готова" и др., но не имеет статусов об оплате! Такая родительская таска собирает все обычные наши таски воедино.
9. Хочу сказать, что самый основной момент - разнести "статус работчего процесса" и "статус оплаты", потому что они попросту независимы! Без второй сущности не обойтись, если мы не хотим все время доставать таски по одной и той же работе, в одном и том же помещении сложными запросами в БД. Это простое логическое их объединение.
10. Так как сложно определиться с терминологием, буду называть далее первую сущность "**Дочерней таской**", а вторую - "**Родительской таской**".

### Ключевые поинты:
1. Технически работу сдельщиков и часовщиков мы реализуем одинаково. Отличиая в ед. изм. работы - час. Разграничние при закрытии нарядов реализуется через выставление фильтра. Работы бывают сдельными (подавляющее большинство) и почасовыми (представлены мелкими доработками). К почасовым работам прибегают не на каждом объекте.
