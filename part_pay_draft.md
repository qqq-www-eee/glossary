...
1. Переход в раздел "Реестры" осуществляется:
    ...
    2. из шахматки через **Боковую панель**, когда в **Левой панели** выбран профиль некоторой работы (но не профиль Замечаний и не профиль Вида работ) - в боковой панели шахматки в этом случае будут отображены ссылки на все реестры в статус "Оплачен", в которые данная работа попала;
    3. пользователь с правом на "Чтение" и пользователь с правом на "Редактирование" одинаково взаимодействуют с контролами как в **Панели действий**, так и в **Боковой панели** и попадают на одинаковую страницу с реестрами, могут одинаково эффективно нажимать на контролы "Создать реестр на выплату" вверху экрана или в заглушке: 
        1. Для пользователя с правом "Чтение" форма, открываемая на Шаге 1 формироdания наряда будет задизейблена и, соответственно, он не сможет выбрать ни одну работу и продлжить формирование наряда.
2. "Чтец" может просматривать список реестров и просматривать содержимое реестров, но не может создавать новый реестр или менять статус существующего реестра.
    1. Создание реестра останавливается на Шаге 1, когда пользователь "Чтец" не может выбрать ни одну работу;
    2. Смена статуса существующего реестра (предположительно) останавлиается на экране просмотра наряда, где контрол смены статуса наряда будет задизейблен [как указано тут](https://zpl.io/9qAW38y).
3. "Редактор" и "Владелец" могут просматривать список реестров, просматривать содержимое реестров, создавать новые реестры и менять статусы существующих реестров.
4. 
