...
1. Для описания частичного закрытия нарядов недостаточно "таски". Наши таски необходимо разделить на две сущности с понятными атрибутами и статусной моделью. Что привело к такой мысли:
    1. Неясно, чем отличаются статусы "Ожидает дефектовку" и "Готово к работе".
    2. Один и тот же статус "Ожидает дефектовку" для созданной и несозданной таски технически отличается. В начале, когда таска не создана, "Ожидает дефектовку" = "Таски нет и статуса нет", а если таска создана (например была указана дефектовка = 0), это становится реальным статусом.
    3. Три серых цвета ни при чем, но хочется их отметить.
    4. Дефектовка - это история, которую делают единственный раз в самом начале объекта, когда принимают его. Из-за передачи фронта работ дефектовка все-таки ближе к атрибутам "работы", чем к атрибутам "помещения", но она точно не относится к сущности, оседающей в закрываемых нарядах. 
    5. При этом новые замечания технадзоры оставляют каждый расчетный период.
    6. Статусы об оплате также работают с периодичность в неделю (что как бы намекает), но главное - тем сущностям, которые попадают в эти частичные наряды и в рамках них закрываются, нужен флаг, отличающий уже оплаченную порцию работы от пока неоплаченной ("статус" либо просто bool).
    7. Ячейка "Вид работ" нужна зеленой тогда, когда по-настоящему закончен целый срез работ. То есть зеленая фасолина должна означать реально сделанную работу. Даже если мы хотим, чтобы ячейка "Вид работ" красилась каждую неделю в знак законченных частичных работ, нам по-хорошему лучше эти два состояния отличать. То есть, например, красить фасолину в синий и отдельно в зеленый. Это позволит пользователю отличать во вкладке обзор работы, которые "Готовы в рамках периода" от тех, что "Совсем прям готовы". Главная идея - статусы надо крепить к работе, не к частичной работе (у нас так и сделано). А оплату при этом никак невозможно отделить от частичной оплаты.
    8. Как понять, что частичная работа готова к оплате? За счет нового промежуточного статуса - это самое просто решение.
2. За частичную работу платят в рамках одного отдельно взятого расчетного периода (и наряда). После закрытия этого наряда для вошедших в него частичных работ, **если их основная работа еще себя не исчерпала, пересоздаются новые сущности частичных работ "на остаток"** - чтобы уже оплаченные работы больше ни в один наряд не попали, но и работа не затерялась. При этом система должна знать, когда остановиться (иначе в конце каждой работы будут вечно висящие таски, которые будут попадаться в нарядах).
    1. Какие частичные работы попадут в наряд? - Те, что находятся в правильном статусе (не в "completed", а скорее в "readyToPay") + у них указан исполнитель + указан объем произведенных работ.
    2. Не смущает ли нас, что фасолины красятся в зеленый и также ячейка "Видов работ" красится в зеленый по одному принципу (статус), а вот на оплату уходят другие (их подмножество, у которых указаны объемы и указаны исполрнители)?
    2. Когда перестаем создавать таски на остаток?
        1. opt1: Хотя значение дефектовки можно спокойно редактировать, при пересоздании частичных работ полагаемся на это поле (тогда Пользователю необходимо подстветит и объяснить эту логику - мол свободный ввод данных в поле дефектовка не значит, что мы на нее не смотрим - мы смотрим). А еще пользователь может захотеть работать в шахматке с ситуацией с контролируемым перерасходом - придется костылить: вводить в дефектовке вводит ненастоящие значения, и все равно потребуется кнопку для создания таски или откатывать до предыдущей версии, правя дефектовку там и пр. Очень непростой вариант, логика перегруженная.
        2. opt2: Всегда пересоздавать таску на остаток, но дать пользователю прямую кнопку, как убить таску.
        3. opt3: Новый статус. Пользователь просто переводит работы в специальный статус "Готово1" (readyToPay), при котором таска будет пересоздаваться на остаток, или в другой статус "Готово2" (completed), при котором таска на остаток пересоздаваться уже не будет. Первая на вкладке "Обзор" покрасит фасолину, скажем, в сине-зеленый; вторая - в ярко-зеленый.
            1. Это позволит потом даже расширить функционал вкладки "Обзор" - позволит пользователю отличать работы, которые "Готовы в рамках периода" от тех, что "Совсем готовы".
        4. Можно при формировании наряда прямо спрашивать - вы эту работу хотите дальше отслеживать? Надо будет все время проставлять либо тогглы, либо чекбоксы. Немного не дружественно. Но и статусы всем проставлять не сильно отличается?
    3. Как вычислять, кого из частичных работ мы в наряд кладем?
        ...
        3. Возможно, пользователю нужна подсказка, при формировании наряда, что эти работы потенциально нужны не в статусе "Готовы к оплате", а, вероятно, должны быть завершены совсем. Либо нужен прямой вопрос - создать вам таску на остаток?
        4. Для каждой таски (частичной работы), которая была создана системой автоматически при завершении наряда, нужен флаг, позволяющий отличить их от тасок, заведенных пользователем. Если пользователь заходит в такую таску, то он видит: где-то горит сообщение о том, что данная работа была создана и заполнена автоматически! Что ему нужно проверить все поля, а именно: исполнителя, объем сделанного (0), объем выданного (0) - то есть как раз именно те поля, что относятся к частичной работе. Как только пользователь нажал в этой работе "сохранить изменения", она перестает считаться сгенерированной системой, больше ничего гореть не будет.
    3. Еще разок: кашу из статусов надо было разделить по трем сущностям: "помещение", "родительская работа" и "частичная работа".
        1. Полный список:
        ...
        2. Распределение:
            ...
            2. Работе достаются: "В работе", "Ожидает технадзор", "Есть замечания", "Завершено" достаются работе. "Готово к оплате" тоже достаются работе. "Ожидает дефектовку" тоже достается работе.
                ...
                2. Важно понять, как именно происходит переход статуса от "Ожидает дефектовку" к "Готово к работе". Первый должен быть дефолтным статусом псевдо-тасок (отображается "статус" несуществующей таски). Второй - реальным статусом тех тасок, которые имеют заполненное поле "Дефектовка" и естественно уже существуют.
