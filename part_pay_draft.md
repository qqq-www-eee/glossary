...
| Термин                        | Значение, синонимичные понятия | Таблица в БД |
| ----------------------------- | ------------------------------ |------------- |
| **Реестр на выплату**         | Сводная таблица с результатами работы мастеров за характерное время с указанием объемов и расценок. Объемы отдельных работ обычно указывают при закрытии **нарядов**, а итоговую сводную сумму по работам для каждого исполнителя - в **реестрах на выплату**. У наc реализовано единое представление с объемами работ и с выплатами, поэтому понятия "формирование реестра" и "закрытие наряда" синонимичны. Корректное - формирование реестра. | **payment_report** |
...
| **Запись о работе** | Определенный объем работы, попавший в реестр; одна строка в таблице реестра на выплату. | **payment_report_task** |
...
2. Реестры на оплату, фактически, сильно сблизились по функциональности с отчетами, но отображают другие поля.
...
4. Наряд невозможно жестко удалить (на текущий момент).

5. Спектр отображаемых данных в наряде (обсуждаются):
    1. Отображаются сейчас:
        1. Название работы
        2. Этаж
        3. Помещение
        4. Исполнитель
...
7. Реестры могут иметь статусы "Сформирован", ~~"Отправлен на оплату",~~ "Оплачен", "Удален".
