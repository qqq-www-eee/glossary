...
| **Расценка**  | Стоимость работы (руб.) за единицу измерения работы (час, м2, пр.). | **payment_report_task** |
...
17. ~~Функционал закрытия реестров будет необходим в мобильном приложении как только будет реализована интеграция с 1C. Он будет работать только если устройство онлайн.~~ Сейчас в мобильных устройствах функционал реестров не реализуется.
...
11. ~~Возможно, в недалеком будущем потребуется разделение прав на редактирование шахмакти, редактирование замечаний, редактирование нарядов".~~
...
1. Переход в раздел "Реестры" осуществляется:
    1. из шахматки через **Панель действий**, по клику на контрол "Закрытие нарядов";
    2. из шахматки через **Боковую панель**, если в **Левой панели** выбран профиль некоторой работы (но не профиль замечаний и не профиль вида работ). В боковой панели шахматки тогда будут отображаться ссылки на все реестры, в которые попала данная работа и которые имеют статус "Оплачен".
    3. "Чтец" может просматривать список нарядов и просматривать содержимое наряда, но не может создавать новый или менять статус наряда.
    4. Пользователь с правом на "Чтение" и пользователь с правом на "Редактирование" одинаково взаимодействуют с контролами как в **Панели действий**, так и в **Боковой панели** и попадают на одинаковую страницу с нарядами. 
...
3. (?) Массовое действие из шахматки: выбрать несколько ячеек и закрыть по ним наряд. 
...
2. **П** переходит в раздел "Закрытие нарядов", где сможет увидеть все созданные реестры и кнопку "Создать реестр". Так как реестров не создавались, сейчас **П** видит заглушку, где дублируется предложение "Создать реестр на выплату". Нажимает на любую из кнопок "Создать реестр на выплату".
