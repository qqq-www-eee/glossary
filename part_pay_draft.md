...
3. ЧАСТИЧНАЯ РАБОТА В НАРЯДЕ (фиксация саб-таски на момент закрытия наряда, в наряде составляет одну строку ~~только статуса об оплате пока нема~~):
...
11. ~~Считаем, что дата создания наряда совпадает с датой окончания расчетного периода.~~
6. ~~Статус "Ожидает дефектовку", лучше бы напрямую значил, что таски пока нет. То есть таска заводилась бы вместе с непустым полем "Дефектовка"? Тоже нет.~~
