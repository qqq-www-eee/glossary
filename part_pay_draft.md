...
| **Расценка**  | Стоимость работы за единицу измерения работы. | **payment_report_task** |
...
8. Система запрещает пользователям (в том числе создателю реестра и владельцу шахматки), редактировать уже существующие реестры во избежание искажения или потери информации в нарядах (реестр - слепок данных шахматки на определенный момент времени).
...
1. Переход в раздел "Реестры"
    1. из меню шахматки
    2. из боковой панели.
2. Формирование нового реестра.
...
3. Просмотр списка реестров.
    1. фыва
    2. фвыа
    3. Фильтрация и сортировка
4. Смена статуса реестра.
5. Отображение **записей об оплате** в **боковой панели**.
    1. asdf]
    2. asdf
    3. Порядок отображения со датам
6. ДОлжно быть можно работу просто уже нафиг закрыть руками, чтобы она в наряды больше не попадала. Почему бы и нет, очень будет удобно.
...
1. Быстродействие - постепенно будет уточняться.
2. Безопасность:
...
6. **П** видит экран, предлагающий ввести расценку для единственного названия работы, для которого нашлись работы. Инпут единственной расценки, с которой **П** теперь работает ("Поклейка обоев почасовая"), не предзаполнен.
