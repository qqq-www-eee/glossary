...
1. Для описания частичного закрытия нарядов недостаточно "таски". Таски необходимо разделить на две сущности с более понятными атрибутами и статусной моделью. Что привело к таким мыслям:
    1. "Ожидает дефектовку" и "Готово к работе" - неясно, чем отличаются.
    2. Статус "Ожидает дефектовку" для созданной и несозданной таски по смыслу отличаются. В начале, когда таска не создана, "Ожидает дефектовку" = "Таски нет и статуса нет", а если таска создана (например была указана дефектовка = 0), это становится реальным статусом.
    3. Три серых цвета - ок?
    4. Дефектовка - это история, которую делают единственный раз в самом начале объекта, когда принимают его. Из-за передачи фронта работ дефектовка все-таки ближе к атрибутам "работы", чем к атрибутам "помещения", но она точно не относится к частично закрываемым нарядам. 
    5. При этом новые замечания технадзоры оставляют каждый расчетный период.
    6. Статусы об оплате также работают с периодичность в неделю, что как бы намекает, но главное - тем сущностям, которые попадают в частичные наряды и в рамках них закрывается, нужен флаг, отличающий оплаченную порцию работы от неоплаченной (либо "статус", либо "булев атрибут").
