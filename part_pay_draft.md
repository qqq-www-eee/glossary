...
1. **Наряд** - таблица, отражающая объёмы выполненных работ за определенный период по всем работникам секции. Закрывается технадзором: через осуществление приемки работ, приемки отработанных замечаний, если они были, фиксации всех фактических показателей работ. Выглядит приблизительно [так]() или [так]().
...
3. **Расчетный период** - период времени, за который составляется наряд и соответствующий реестр на выплату. Как правило, по бизнесу пользователю расчетный период заранее известен (7 дней) и не изменяется. Системе заранее неизвестен - подбирается автоматически при формировании наряда.
4. **Работа** - проведение определенного наименования работ в определенном помещении. Может длиться один или нескольких расчетных периодов. Работа может быть разбита на "частичные работы" (они же "таски", "задачи"). Не путать с наименованием работ, которые приведены в справочнике. Работа однозначно определяется по комбинации **work_id** + **storey** + **room_group** и по сути есть слайс помещения под конкретную работу. 
...
1. Чаще всего, расчетный период равняется неделе.
2. Дефектовка вообще происходит на объекте только один раз, в самом начале самой первой работы. При этом новые замечания технадзоры оставляют каждый расчетный период. Налицо явная нестыковка в статусах, еще не касаемо частичных оплат и статусов об оплате.
...
4. Нам недостаточно для описания частичных работ одной только таски, очень неудобно, на грани невозможного. Плюс становятся все более очевидными требования к статусам. В частности, "Ожидает дефектовку" и "Готово к работе" - непонятно чем отличаются. Еще три серых цвета, конечно, вопросы вызывают.
5. За частичную работу (то есть "таска") хотят заплатить в рамках одного отдельно взятого расчетного периода (и наряда). После закрытия этого наряда для вошедших в него частичных работ, если основная работа еще себя не исчерпала, пересоздадутся новые сущности частичных работ "на остаток" - чтобы уже оплаченные работы больше ни в один наряд не попали.
6. Работа имеет статусы "Дефектовка"/"В работе"/"Замечания"/"Готова" и др., но не имеет статусов об оплате! Такая вот "родительская таска" собирает все таски воедино. Самый ключевой момент - разнести "статус рабочего процесса" и "статус оплаты", потому что они попросту независимы. Вторая сущности нужна для нарядов внутри, внешняя для нарядов снаружи (вкладка "Обзор" там и так далее).
7. Считаем, что дата создания наряда совпадает с датой окончания расчетного периода.
...
9. Смежным лейтмотивом идет вложенность/невложенность project, storey, room_goup. Таски должны убиваться и создаваться на остаток при частичном закрытии наряда. А вот новая сущность Работа должна жить до тех пор, пока живут три сущности:
    1. ее этаж
    2. ее рум_груп
