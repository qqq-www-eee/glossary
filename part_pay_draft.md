...
1. **Наряд** - таблица, отражающая объёмы выполненных работ за определенный период (часто неделя) по всем работникам секции. Закрывается технадзором: он осуществил приемку работ, приемку отработанных замечаний, если были, и зафиксировал все фактически показатели работы. Выглядит приблизительно [так]() или [так]().
...
3. **Расчетный период** - период времени, за который составляется наряд и реестр на выплату. Как правило, по бизнесу пользователю заранее известен и не изменяется (7 дней). Системе заранее неизвестен и подбирается автоматически при формировании наряда.
4. **Работа** - процесс проведения определенного наименования работ в определенном помещении. Может длиться один или нескольких расчетных периодов. Работа может быть разбита на "частичные работы" (они же "таски", "задачи"), работа - это "глобальная таска". Не путать с наименованием работ, которые приведены в справочнике.
...
5. **Частичное закрытие наряда** (иногда **процентовка**) - процесс сбора и подготовки информации по объекту для взаиморасчета с мастерами за 1 расчетный период (за 1 частичную работу).


### Интересные факты и важные пункты

1. Дефектовка происходит только один раз в начале работы. Новые замечания технадзоры оставляют каждый расчетный период.
2. Строительные работы бывают сдельными (подавляющее большинство) и почасовыми (представлены мелкими доработками). К почасовым прибегают не на каждом объекте. Технически работа сдельщиков и часовщиков реализуется одинаково, отличие в единицах измерения - у сдельных работ это всегда час. Разделять сдельные и почасовые работы при закрытии нарядов можно через выставление фильтра по единицам измерения - то есть делать, в частности, два отдельных наряда.
3. Нам недостаточно для описания частичных работ одной только таски, очень неудобно.
4. За частичную работу (то есть "таска") хотят заплатить в рамках одного отдельно взятого расчетного периода (и наряда). После закрытия этого наряда для вошедших в него частичных работ, если основная работа еще себя не исчерпала, пересоздадутся новые сущности частичных работ "на остаток" - чтобы уже оплаченные работы больше ни в один наряд не попали.
5. Работа однозначно определяется по комбинации **work_id** + **storey** + **room_group** и по сути есть слайс помещения под конкретную работу. Имеет статусы "Дефектовка"/"В работе"/"Замечания"/"Готова" и др., но не имеет статусов об оплате! Такая вот "родительская таска" собирает все таски воедино. Самый ключевой момент - разнести "статус рабочего процесса" и "статус оплаты", потому что они попросту независимы. Вторая сущности нужна для нарядов внутри, внешняя для нарядов снаружи (вкладка "Обзор" там и так далее).
1. Считаем, что дата создания наряда совпадает с датой окончания расчетного периода.
2. Наряд - неизменяемая штука (ни пользователем, ни системой). ~~В ней, возможно, можно будет редактировать коммент-название и статус, но не факт, что они появятся.~~ 
3. Смежным лейтмотивом идет вложенность/невложенность project, storey, room_goup. Таски должны убиваться и создаваться на остаток при частичном закрытии наряда. А вот новая сущность Работа должна жить до тех пор, пока живут три сущности:
    1. ее этаж
    2. ее рум_груп
    3. ее наименование работ.
Зачем это? Это позволит делать УДОБНЫЙ селект по "всем частичным работам, относимых к данному наименованию работ в данном помещении". По айдишнику, а не по совокупности общих характеристик (по идее мы их можем вычислить по одному и тому же помещению и одному и тому же наименованию работ. Помещение, в свою очередь, скоро будет определяться через этаж + room_group. По айдишнику куда как проще вроде бы?). И главное - статусы мы хотим прикрепить к РАБОТЕ, а не к ТАСКЕ? А как тогда понять, что частичная работа готова к оплате? ПРоставлять как ни в чем не бывало, а она сама будет съезжать с помощью системы на какой-то статус, неизвестно какой? Ожиданеие дефектовки? Или в работе?
9. А как полномочить систему пересоздавать таску или нет? Если полностью выполнена вся дефектовка, или может у нее перерасход уже случился? Вот когда она в статусе "Готово", мы больше частичные работы не пересоздаем. Вот зачем нужен этот статус.
10. А как тогда вычислять, кого мы в наряд кладем? А вот так. По статусу либо по дефектовке.
4. А вот интересно, как создается работа. Она создается только тогда, когда пользователь тыкнул в шахматке в конкретное помещение, потом ввел какие-нибудь данные, а потом
