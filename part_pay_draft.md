...
7. Перерасход - что такое? Может ли быть запланированный перерасход? Смотря что такое перерасход, если нормируемся на дефектовку - то да. Если на план, то, наверное, это скорее ошибка - зачесть работы и не учесть, куда взяты материалы. 
8. ыва
9. фыва

### Тезисы

1. Нам недостаточно для описания частичных работ одной только таски. Таски необходимо расслоить на две сущности с более понятными атрибутами и статусной моделью.
    1. В частности, "Ожидает дефектовку" и "Готово к работе" - непонятно чем отличаются.
    2. Еще три серых цвета, конечно, вопросы вызывают.
    3. **Дефектиовка - это характеристика родительской работы** в лучшем случае (близко к тому, это вообще характеристика помещения). Из-за передачи фронта работ, допустим, дефектовка все-таки относится к работе, но точно не к той сущности, которая попадает в частично закрытые наряды. Дефектовка - это история, которую делают в самом начале объекта, когда принимают его.
    4. При этом новые замечания технадзоры оставляют каждый расчетный период.
    5. Со статусом об оплате работают с периодичностью недели. Это должно быть стсатусом той сущности, которая попадает в частичные наряды и в рамках них закрывается. В общем, налицо явная нестыковка в статусах, которые сейчас слеплены воедино.
    6. Главное - статусы мы хотим прикрепить к РАБОТЕ, а не к ТАСКЕ. Почему? потому что "Вид работ" будет становиться зеленым, когда закончен целый срез работ. Поэтому Зелеными фасолины становятся только в самом конце. Но как же тогда понять, что частичная работа готова к оплате? Ну за счет промежуточного другого статуса.
2. За частичную работу платят в рамках одного отдельно взятого расчетного периода (и наряда). После закрытия этого наряда для вошедших в него частичных работ, **если основная работа еще себя не исчерпала**, пересоздаются новые сущности частичных работ "на остаток" - чтобы уже оплаченные работы больше ни в один наряд не попали. Но система должна знать, когда остановиться, иначе в конце каждой работы будут вечно висящие мертвым грузом таски, которые еще, возможно, все время будут попадаться в нарядах.
    1. Какие частичные работы попадут в наряд? Только те, что находятся в правильном статусе (еще обсудим, в каком), и еще у них указаны исполнитель и объем работ.
    2. Когда перестаем создавать таски на остаток? Хотя значение дефектовки можно спокойно редактировать, но при пересоздании тасок полагаемся именно на это поле.
    3. Пользователю необходимо подстветитьи объяснить эту логику - что мол свободный ввод данных в поле дефектовка не значит, что мы на нее не смотрим. Мы смотрим. 
    4. Альтернативой может быть - всегда пересоздавать таску на остаток, но дать пользователю прямую кнопку, как убить таску.
    ...
    17. Как вычислять, кого из частичных работ мы в наряд кладем? А по статусу ready. А новую частичную работ на остаток создаем? - смотрим по по дефектовке.
3. Еще раз. Кашу из статусов надо распихать по трем сущностям: "помещение", "родительская работа", "частичная работа".
    1. Полный список:
        ...
        8. Оплачено
    2. Распределение:
        1. Частичной работе точно достается - "Готово к оплате", "Оплачено". Далее станет понятно, что ей же нужен еще статус "Не готово к оплате, еще в работе".
        2. "В работе", "Ожидает технадзор", "Есть замечания", "Завершено" могут достаться либо работе, либо частичной работе в зависимосит от пожелания бизнеса. 
        2. "Дефектовка" - параметр для родительской таски, так как хотим относить к каждому "типу работ + помещение" отдельную дефектовку, в частности, для передачи фронтов работ. Поэтому "ожидает дефектовку" и "Отдефектовано" (= готов к работе) достанутся родительской работе. Это может быть просто заполненное или не заполненное поле.
        3. "Квартире" не досталось ничего.
    3. Возникают ограничения:
        1. Сначала создается шахматка, и в ней вообще нет работ. 
        2. Если уже здесь выйти на вкладку обзор - там у нас будут пустые серые фасолины (например 5 шт, по числу наименований работ в группе "Вид работ") (~~подписана первая работа в этой группе? Видимо, идущая чисто по алфавиту, даже не по бизнес-логике. Зачем мы показываем рандомную работу? Это баг~~). Если все фасолины статусе "ожидает дефектовку" (пустые серые) И поля в них не были заполнены, то в правой панели должна показывать заглушка про отсутствие работ.
        3. Как только хотя бы одна работа в виде работ получает заполненное поле, создается работа + частичная работа. Все "объемы" и "выдано" идут в частную работу, а значения по дефектовке должны относиться к родительской работе. Или как только хотя бы одна работа получает статус, отличный от "ожидает дефектовку", тоже создается работа + частичная работа. Вопрос только смысла этого статуса, если цифра дефектовки не указана?
        ...
        4. Кстати довольно странно выглядит, что одна и та же серая фасолина может являться как штукой без таски, так и штукой с таской.
        5. ~~Число фасолин, кстати, вычисляется почему-то на основе работ, уже представленных в шахматке. То есть если в виде работ у нас 6 наименований работ, то 6 будут только если заполнены все 6 из них хоть где-то. Наверное, это тоже баг.~~
        6. Потом у самих фасолин начинают меняться статусы. Этот статус может стать "Готово к работе" (дефектовку мол провели), потом в работе, потом ожидает технадзор, потом замечания, потом снова в работе-ожидает_тезнадзор-замечания, так несколько раз и потом только в самом конце это должно быть "Готово". Это готово, фактически, нужно именно для вкладки "Обзор". 
        7. Поэтому статусная модель предполагается очень даже похожей.
