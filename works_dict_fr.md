...
> Если привязывать работу к системному списку работ (sys_work_list) вместо списка работ шахматки (work), то при удалении строки из work работа (task) все еще будет ссылаться на корректное наименование в sys_work_list. Данные не теряются, получится soft-delete для работ, и при добавлении работы назад все работы в шахматке заресторятся. Но:  
> 1. Мы не хотим перегружать логику пользователю  
> 2. У пользователя будет версионирование и откат.  
> 3. Если связанные с наименованием работы таски не перестанут существовать, они могут продолжить появляться в нарядах. Тогда у работы напрашивается дополнительный статус "Удалена" (но статус работы - это немного про другое), поэтому просто удаляем.  
> 
> Так что работы (таски) привязываем к work, и удаление строки оттуда приведет к жесткому удалению соответствующих работ (тасок). Если пользователю очень надо, пусть держит наименование работ дальше.
