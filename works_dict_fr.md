...
> 1. Не хотим перегружать логику пользователю  
> 2. У пользователя будет версионирование и откат.  
> 3. Если связанные с наименованием работы таски не перестанут существовать, они могут появляться в нарядах. Тогда у работы напрашивается дополнительный статус "Удалена" (но статус работы - это немного про другое), переусложнение. Если пользователю очень надо, пусть держит наименование работ дальше.
> 
> Так что предлагается работы (таски) привязывать к work, и удаление строки оттуда приведет к жесткому удалению соответствующих работ (тасок) (либо в тасках проставляется null - в любом случае, нам эти таски, считаем, не нужны).

### Функциональные требования

1. При переходе в шахматку (в объект):
    ...
    3. если в списке работ шахматки нет ни одной работы, пользователь в разделе "**Список работ**" видит в области системных работ перечень наименований всех работ (sys_work_list, [сам список тут](/Дефолтные-работы.pdf)), а в области работ шахматке - заглушку.
2. Пользователи могут добавлять новые строки в системный список работ (sys_work_list), только через заведение их в список работ шахматки (work) (попадает автоматически);
