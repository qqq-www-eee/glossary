...
- Удалить поле code? в таблице role
<br>
...
> - Пользователей (прорабы, технадзоры, начальники участка) и испольнителей (рабочие) необходимо хранить в двух разных таблицах.
> 
> - Мобильного приложения для рабочего, скорее всего, не будет.
> 
> - Доступы необходимо отвязать от должностей пользователей.
> 
> - Не только ФИО, но и Должность пользователя в системе отображается сквозным образом.
> 
> - Мы не хотим заставлять пользователя вести учет Фамилий, Имен, Отчеств, и поэтому предлагается везде в системе перейти к единому полю - "ФИО", в том числе на страничке "Мой профиль". Это отразится на БД.

Основные тезисы по доступам:

> - Приглашенные пользователи с правом "Чтение" могут только просматривать шахматку (объемы работ, исполнители, этажи) и просматривать замечания.
> - Приглашенные пользователи с правом "Редактирование" могут редактировать шахматку, редактировать справочники, создавать/редактировать/удалять **свои** замечания, **а также менять статусы чужих замечаний**.
> - Создатель шахматки может делать все, в том числе редактировать и удалять чужие замечания и настраивать доступы.


Back-end

> - Необходимо реализовать отедльную таблицу для исполнителей. Структура похожа на user (см. ниже).
> - Необходимо отвязать понятие "Должность" от доступов (role) пользователей:  
Из таблицы  **project_role** убрать все записи, кроме "Чтение" и "Редактирование". Почти все оставльные строки необходимо вынести в отдельную таблицу с должностями (**user_job_title_d** - пример имени, справочник, см. ниже). В этой же таблице **project_role** нужно удалить поле status (легаси). 
> - Таблицу **permissions**, вероятно, необходимо дропнуть. (role и permissions вдвоем доставляют очень близкий по смыслу функционал регулирования доступов, и сейчас мы имеем возможность и желание от этого дробления уйти).
> - Таблицу **role_permission**, вероятно, необходимо дропнуть.  
> - В таблице **user** нужно убрать поля gender, status, first_name, middle_name, last_name. Добавить поле full_name (которое можно заполнить конкатенацией из фамилий-имен-отчеств) и добавить поле job_title с uuid должности из нового справочника (**user_job_title_d**).  
>
...
Design + front
> - В экране редкатировании списка "Исполнители проекта" привести где-то поясняющий текст "Исполнители проекта не имеют доступа к шахматке";  
> - В экране "Мой профиль": добавить поле "Должность" на front-end в виде селектора (справочника) с единичным выбором (экран "Моя учетная запись");  
> - В экране "Мой профиль": заменить "Фамилию", "Имя", "Отчество" на одно обязательное поле - "ФИО" для предсказуемого поведения системы. 
...
> - Положить в бэклог задачу по внесеню никнеймов в систему (говорим о ФИО и о должности). От никнеймов нам не уйти, даже если исполнителей мы храним отдельно от пользователей. Вероятно, надо будет в связь **project_role** добавить ФИО данного пользователя в данном проекте и Должность данного пользователя в данном проекте. Изменения в эту таблицу должен вносить только содатель. 
...
| created       |   |
| updated       |   |
нереальный, поэтому можно задать job_title как сквозной атрибут пользователя.
