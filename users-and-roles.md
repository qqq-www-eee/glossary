...
> - Необходимо отвязать "Должность" от захарадкодженых доступов;
> - Необходимо привязать доступы напрямую к uuid пользователя;
> - Поле "Должность" на front-end реализовать как селектор (справочник) с единичным выбором, поправить экраны;
> - У замечаний должен появиться автор - один технадозор не всегда может править заметки другого технадзора, но часто может менять им статус (резолвить).
> - В таблице **user** убрать поля gender, status, first_name, middle_name, last_name. Добавить full_name, которое можно заполнить конкатенацией из фамилии-имени-отчества.
...
> - В таблице **project_role** удалить поле status. 
> - На экранах редкатировании списка "Исполнители проекта" привести текст о том, что "Все исполнители проекта смогут просматривать шахматку, если зарегистрируются в системе". 
> - 
...
### Какие id необходимо отразить в селекторе:

Должность: справочник, пока без озможности добавления/удаления пользовательских должностей.
