...
> - вопрос либо перетекания контактов в пользователи, либо двойственности одного и того же номера телефона в системе (шахматку менял пользователь, а в списки я добавляю просто текст), а также вопрос корректировки пользователем своих данных и подтягивания их в списки. 
> - плодим сущности без надобности, создаем много дублей (но не критично);
...
> - есть шанс все-таки запутать пользователя.
> 
> Аргументы за:
> - Простота реализации;
...
>   - Даже если коллективное редактирование ок, то кто именно вправе задавать сквозное имя для "пользователя-контакта", которого в разных шахматках вписывают в свои списки разные пользователи - все, никто, система? Какое имя более правильное - первое или последнее? И так далее.
...
> - Никнеймы решат проблемы разных отображений имени в системе - тремя полями и одним полем. Тремя полями заполняем пользователя (доступно только на экране "Учетная запись") и не показываем пока нигде, а одним полем - во всех остальных местах.
...
> - Необходимо сделать таблицу, которая будет содержать отражение реальных системных пользователей на профиль текущего пользователя (нет смысла привязывать к каждому отдельному проекту текущего пользователя, есть смысл привязывать к текущему пользователю).
> - "ФИО", но и "Должность" пользователя в системе отображаются сквозным образом.
> - Мы не хотим заставлять пользователей вести учет Фамилий, Имен и Отчеств, поэтому предлагается везде в системе перейти к единому полю - "ФИО" (в разделе "Мой профиль" можно поменять, но давайте пока оставим три поля, потому что таблица никнеймов позволит нам это провернуть).
