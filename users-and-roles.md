...
>
> > Таким образом, как минимум **технадзоры не позволяют полностью отделить пользователей от контактов**. Список шахматки "Технадзор" не подразумевает подтягивание данных из разных мест БД и сложную логику для их обработки, избежания дублей и т. д.
...
Решение 2 
> Другой вариант - мы вообще не отличаем пользователей и контакты и храним их в единой таблице БД. Возникают вопросы:
...
>   - Может ли мы распространять чувствительные данных по фио, телефону, телеграму и т. д.

## Вывод:

Решение 3
> Без решения задачи по вводу никнеймов в систему (говорим пока о ФИО и о Должности) не обойтись.  
> Никнеймы потребовались бы, даже если **контакты** хранить отдельно от пользователей - потому что сквозное изменение имен все равно бы проявилось, просто только для пользователей (то есть технадзоров как минимум).
> Заодно никнеймы еще и решают такую проблему как...
>
...
> **Изменения в эту таблицу (или другое место хранения никнеймов) должен вносить только содатель**. 
