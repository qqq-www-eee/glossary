...
> - Пользователей (прораб, мастер участка, технадзор, начальник участка) и испольнителей (рабочий, бригадир)  **необходимо хранить в двух разных таблицах**.
> 
> - В таблице, хранящей исполнителей, несколько записей может быть связано с одним и тем же человеком; одноко в рамка одного проекта исполнители (номера телефонов) уникальны, т. е. "project_uuid + phone" является для таблицы уникальной комбинацией.
> - Мобильного приложения для рабочего, вероятно, не будет.
> - Доступы необходимо отвязать от должностей пользователей.
> 
> - Не только "ФИО", но и "Должность" пользователя в системе отображаются (будут отображаться) сквозным образом.
> - Мы не хотим заставлять пользователя вести учет Фамилий, Имен, Отчеств, и поэтому предлагается везде в системе перейти к единому полю - "ФИО", в том числе на страничке "Мой профиль" (отразится на БД).
> - В одном и том же помещении могут быть замечания, оставленные разными технадзорами. Какая-либо связь комментирующих с тем технадзором, который "закреплен" за помещением в шахматке, отсутсвтует.
> - Для каждого среза работ у нас свои замечания. Переключение между видами работ сейчас в замечаниях не предусмотрено - они одни на все работы. ТУТ НАДО РЕШИТЬ, ИХ В ВИДЕ ОТДЕЛЬНОЙ ЗНАЧИТ ВКЛАДКИ МЫ ДЕЛАЕМ? Нам нужно решить, хотим мы категорию отображать на вкладке замечаний уже или мы хотим изначально навигироваться по работам, а замечания вынести в **Панель действий**. 
> - Замечания очень просятся в панель действий.

Основные тезисы по доступам:

> - "Чтение". Приглашенные пользователи с правом "Чтение" могут только **просматривать шахматку** (объемы работ, исполнители, этажи) и **просматривать замечания**.
> - "Редактирование". Приглашенные пользователи с правом "Редактирование" могут редактировать шахматку, редактировать справочники, создавать/редактировать/удалять **свои** замечания, **а также менять статусы чужих замечаний**.
> - "Владелец". Создатель шахматки может делать все, в том числе редактировать и удалять чужие замечания и настраивать доступы.

Back-end

> - Необходимо реализовать отедльную таблицу для исполнителей со структурой, похожей на user (см. ниже).
> - Необходимо отвязать понятие "Должность" от доступов (role) пользователей:  
Из таблицы  **project_role** убрать все записи, кроме "Чтение" и "Редактирование". Почти все оставльные строки необходимо вынести в отдельную таблицу с должностями (напр., **user_job_title_d**, справочник, см. ниже). В этой же таблице **project_role** нужно удалить поле **status** (легаси). 
...
> - В таблице **user** нужно убрать поля gender, status, first_name, middle_name, last_name. Добавить поле full_name (которое можно заполнить конкатенацией из фамилий-имен-отчеств) и добавить поле job_title с uuid должности из справочника **user_job_title_d**.  
>
...
> - Должность должна сквозным образом отражать данные в системе.
> - Необходимо добавить возможность корректировки исполнителя - без удаления строки.

Design + Front-end
> - В экране редкатировании списка "Исполнители проекта" привести текст, поясняющий, что "Исполнители проекта по умолчанию не имеют доступа к шахматке" (или похожий);  
> - В экране "Мой профиль": необходимо добавить поле "Должность" на front-end в виде селектора (справочника) с единичным выбором (экран "Моя учетная запись");  
...
> - Положить в бэклог задачу по внесеню никнеймов в систему (говорим о ФИО и о должности) - они потребуются, даже если исполнителей хранить отдельно от пользователей. Вероятно, надо будет в связь **project_role** добавить ФИО данного пользователя в данном проекте и Должность данного пользователя в данном проекте. Изменения в эту таблицу (или другое место хранения никнеймов) должен вносить только содатель. 
