# Пользователи, должности, доступы, приглашения
...
> - Необходимо отвязать понятие "Должность" от доступов (role) пользователей.  
Из таблицы  **project_role** необходимо убрать все строки, кроме "Чтение" и "Редактирование". Почти все оставльные строки необходимо вынести в отдельную таблицу с должностями (например, **user_job_title_d**, справочник, см. ниже). В таблице **project_role** нужно удалить поле status (легаси). 
>  
> - Таблицу **permissions**, вероятно, необходимо дропнуть. (role и permissions вдвоем доставляют очень близкий по смыслу функционал регулирования доступов; сейчас мы имеем возможность и желание от этого дробления уйти).
>  
> - Таблицу **role_permission**, вероятно, необходимо дропнуть.  
> 
> - В таблице **user** нужно убрать поля gender, status, first_name, middle_name, last_name. Добавить поле full_name (которое можно заполнить конкатенацией из фамилий-имен-отчеств) и добавить поле job_title, в котором будет оражен uuid должности из нового справочника (**user_job_title_d**).
> 
> - Предлагается в таблицах **role** и **permission** перейти от айдишников _serial4_ в пользу _uuid_.
> 
> - 
> - От никнеймов нам не уйти, даже если исполнителей мы храним отдельно от пользователей. Потому что у пользователя не должен смениться ФИО технадзора в шахматке просто так. Поэтому в связь project_role надо добавить ФИО данного пользователя в данном проекте и Должность данного пользователя в на данном проекте. Изменения в эту таблицу должен вносить только содатель. 
> - При созранении данных из экрана "Моя учетная запись" вместо Фамилии, Имени, Отчества сохранять единое поле "ФИО", а также сохранять новое поле "Должность".
> - необходимо реализовать отедльную таблицу для исполнителей. Структура похожа на user, но просто храним отдельно от пользователей.
> - ~~В будущем у исполнителей может быть словарь из своих должностей - "бригадир" и "рабочий"~~.
> - Поле Роль для пользователя истемы, считаем, работает сквозным образом, как и ФИО.

Design + front
> - На экранах редкатировании списка "Исполнители проекта" привести где-то текст "Исполнитель проекта не сможет видеть шахматку, пока не получит приглашение";
> - В экранах "Моей учетной записи": добавить поле "Должность" на front-end в виде селектора (справочника) с единичным выбором (экран "Моя учетная запись");
> - В экранах "Моей учетной записи": заменить "Фамилию", "Имя", "Отчество" на одно обязательное поле - "ФИО" для предсказуемого поведения системы. 
> - Нужно убрать из настроек доступа поле "Роль".
> - Когда на проект добавляется технадзор или исполнитель, при вводе номера телефона нужно видеть, как зовут в системе этого человека (поиск по таблицам user и ).
> - Нужны разные экраны для людей с разными доступами - а именно, нужно отличать функционал добавления новых записей в справочники из панели действий.
> - Наверное, даже если считать, что наши справочники, как и настройки, никто менять не может, тогда если ты работаешь на чужой шахматке при выборе исполнителя ты нового добавить не можешь, а если работаешь на своей шахматке при выборе исполнителя нового добавить можно.
> - Пока должности нигде не отражаем, но это желательно будет сделать позже;
> -  
> -  Нужно убрать из настроек доступа поле "Роль"
...
> - Один технадозор не всегда может править заметки другого технадзора;
> - Один технадзор всегда может менять статус заметок (резолвить) другого технадзора;
> - Если в мои шахматки подтягивается ФИО пользователя, которое он сам указывает в аккаунте для себя, то я никак не защищен от того, что мастер "Авакумов" у меня каждый день будет отображаться, то как "Зайка", то как "Милый". Решения тут 2 - либо исполнителей вообще не считать за пользователей, либо считать их другим типом пользователей, либо просто ввести таблицу никнеймов. Иначе есть риск, что однажды утром у меня будет целая бригада "Ивановых". Опять же тут можно откатиться. 
ненормально, когда я даю доступ к шахматке Иванову Ивану Ивановичу, а там сразу же высвечивается Иванова-карамзина М.Ю., например, - сразу же после сохранения. Это странно,и они еще и могут самопроизвольно меняться. 
> - пока оставляем сквозное ФИО и должности пользователей. Но в будущем алиасы(никнеймы) появятся.

> - положить в бэклог задачу по внесеню никнеймов в систему! очень делаетльно (говорим о ФИО и о должности)
...
Редко когда человек бывает на двух объектах

### Таблица project_role

Берем должность пользователя на проекте из этой таблицы.
Накаких статусов, кроме "активно", у записей нет, поэтому статус можно дропнуть, а при смене роли пользователя - делать UPDATE существующей строки.
...
1. Остается без изменений в структуре, однако будет отредактировано содержимое.  
2. Должностей будет в 2 раза меньше, уйдут дубли, поправится нейминг.
3. Должность - справочник, пока без возможности добавления/удаления пользовательских должностей.
> - В таблице **role** есть "Прораб" и "Произовдитель работ" - задублированная информация. Обычно пользователи предпочитали указать "Прораб" с id = 3, значит "Производителя работ" (с id = 15) необходимо удалить.

##### Какие id необходимо отразить в селекторе (и оставить в таблице):

| Должность в UI     | role.id в БД сейчас |
| ------------------ | ------------------- |
| Начальник участка  | 9                   |
| Прораб             | 3                   |
| Технический надзор | 6                   |
| Мастер участка     | 13                  |
| Бригадир           | 20                  |
| Рабочий            | 4                   |
| Сметчик            | 5                   |
...
Предлагается уйта от текущих serial4 к uuid.
> - ~~В идеале в таблице **role** "Технический надзор" надо переименовать в "Инженер строительного контроля", потому что технадзор - термин, перекочевавший из СССР~~.

### Таблица permission

ТАБЛИЦУ НУЖНО КАТЕГОРИЧЕСКИ СОКРАЩАТЬ, если нигде не используется 

Предложение:

<img width="200" alt="image" src="/mermaid-diagram-2023-01-20-104232.png">

uuid или _serial4 - открытый вопрос.

| Функциональность | Комментарий | code |
| - | - | - |
| Замечания | Просмотр чужих замечаний (свои всегда можно смотреть) | COMMENTS_R |
| | - Создание, редактирование (в т. ч. управление файламии), удаление замечаний, смена статуса **своих** замечаний | COMMENTS_CRUD |
| | - Смена статуса чужих замечания | COMMENTS_STATUS_U |
| Шахматки (статусы, объем работ) | Просмотр шахматки | CHESS_R |
| | - Редактирование шахматки  | CHESS_CRUD |
| Списки (справочники)   | Просмотр  | CHESS_R
...
Предложение по реализации наполнения данного справочника:
